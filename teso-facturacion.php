...
<?php
	...
	$sql="SELECT MAX(codigo_factura) FROM tesofacturacion";
...
<form name="form2" method="post" action="teso-facturacion.php">
...
<table class="inicio">
			...
			<select name="tipomovimiento" id="tipomovimiento" onKeyUp="return tabular(event,this)" onChange="validar()" style="width:20%;" >
				<?php 
					...
					if($num==1){
						...
						$sql="SELECT codmov,tipomov from permisos_movimientos WHERE usuario='$user' AND estado='S' AND modulo='4' AND transaccion='THA' ";
						$res=mysql_query($sql,$linkbd);
	...
	<table class="inicio">
	    <tr>
    	  	<td class="titulos" colspan="11">Proceso de Facturacion</td>
		...
		<tr>
      	<tr>
        	...
        	<td>
			<td class="saludo3" style="width: 10%">Tipo: </td>
				<td style="width: 15%">
					<select id="tipo" name="tipo" style="width:95%">
	...
	if($_POST[buscaproc]=="1"){
		...
		$sql="SELECT rangovigencia,codcatastral,predial,bomberil,ambiental,valortotal,tercero,tipo FROM tesofacturacion_det WHERE codigo_factura='$_POST[codigo]'  ";
		$res=mysql_query($sql,$linkbd);
		while($row = mysql_fetch_row($res)){
      		...
      		<?php		 	
				...
				if($_POST[oculto]==1){
					...
					if($_POST[tipomovimiento]=="101"){
						$sqlr="select tesopredios.cedulacatastral from tesopredios where tesopredios.estado!='N' AND tesopredios.tipo='$_POST[tipo]' AND tesopredios.zona='$_POST[zona]' AND tesopredios.estado!='N' AND (EXISTS (SELECT 1 FROM tesoprediosavaluos WHERE tesoprediosavaluos.codigocatastral = tesopredios.cedulacatastral AND tesoprediosavaluos.vigencia='$anoact' AND tesoprediosavaluos.pago='S' AND tesoprediosavaluos.estado!='N' ) OR EXISTS (SELECT 1 FROM tesoacuerdopredial WHERE tesoacuerdopredial.codcatastral = tesopredios.cedulacatastral AND tesoacuerdopredial.estado!='N')) GROUP BY tesopredios.cedulacatastral HAVING tesopredios.cedulacatastral!='' LIMIT 0,100";
						$res=mysql_query($sqlr,$linkbd);
						while($row = mysql_fetch_row($res)){
							$sql1="SELECT COUNT(*) FROM tesoprefacturacion_det WHERE tesoprefacturacion_det.codcatastral='$row[0]' AND NOT EXISTS (SELECT 1 FROM tesoprefacturacion WHERE tesoprefacturacion.tipomov='301') ";
...
if($_POST[oculto]==2){
	...
	if($_POST[tipomovimiento]=="101"){
		$sql="INSERT INTO tesofacturacion(codigo_factura, fecha_factura,totpredial,totbomberil,totambiental,valortotal,tipomov,usuario,vigencia,estado) VALUES ($_POST[codigo],'$fecha',".$_POST[totpredial].", ".$_POST[totbomberil].",".$_POST[totambiental].",".$_POST[totalpred].",'".$_POST[tipomovimiento]."','$user','$vigencia','S')";
		...
		for($i=0; $i< count($_POST[seleccion]);$i++ ){
			try{
				if(!empty($_POST[seleccion])){
					...
					$sql="INSERT INTO tesofacturacion_det (codigo_factura, rangovigencia, codcatastral, predial,bomberil,ambiental,valortotal) VALUES ($_POST[codigo],'".$_POST[rangovig][$i]."','".$_POST[codcatastral][$i]."','".$_POST[predil][$i]."','".$_POST[bomberil][$i]."','".$_POST[ambiental][$i]."','".$_POST[total][$i]."')";
					$res = mysql_query($sql,$linkbd);
					if(!$res){
			...
			echo "<script>despliegamodalm('visible','2','Se ha guardado exitosamente el proceso'); </script>";
		 }
		} 
	}else{
		$sql="INSERT INTO tesofacturacion(codigo_factura, fecha_factura,rangovigencias,totpredial,totbomberil,totambiental,valortotal,tipomov,usuario,vigencia,estado) VALUES ($_POST[codigo],'$fecha','$rangovigencias',0, 0,0,0,'".$_POST[tipomovimiento]."','$user','$vigencia','R')";
		$res = mysql_query($sql,$linkbd);
		if($res){
...
function registrarDetallePredial($ind,$iddet){
	...
	for($i=0; $i < count($objunserializado); $i++ ){
		...
		$sql="INSERT INTO tesofacturacion_predial(codigo_det,codcatastral,vigencia,predial,descpredial,intpredial,bomberil,intbomberil,ambiental,intambiental,descuentos,total) VALUES ($iddet,'$codcatastral','$vigencia',$predial,0,0,$bomberil,0,$ambiental,0,0,$total)";
		$res = mysql_query($sql,$linkbd);
		if(!$res){
			echo "<script>despliegamodalm('visible','2','Error al insertar el registro $codcatastral'); </script>";
			break;
