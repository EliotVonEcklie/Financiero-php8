...
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A1', 'Registros Presupuestales Reversados');
...
$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue('C2', 'Valor RP')
            ->setCellValue('D2', 'Valor Reintegrado')
            ->setCellValue('E2', 'Reintegrado Por')
            ->setCellValue('F2', 'Detalle')
            ->setCellValue('G2', 'Fecha Reintegro');
$crit1=" ";
$crit2=" ";
$crit3=" ";
$vig=vigencia_usuarios($_SESSION[cedulausu]);
...
{	
	...
	$crit3=" AND TB1.fecha between '$fechai' and '$fechaf'  AND TB1.tipo_mov IN (401,402) ";
}
$sqlr="SELECT TB1.* FROM pptorp TB1 WHERE TB1.estado!='N'  $crit1 $crit2 $crit3 ORDER BY TB1.consvigencia";
...
while ($row =mysql_fetch_row($resp))
{
    ...
    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2,$i,(float)$row[6]);
...
header('Content-Disposition: attachment;filename="presu-rpr.xls"');
