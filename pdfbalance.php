...
while($row=mysql_fetch_row($res))
 ...
 //$nalca="RIVERA RINCON JAIRO ";
	...
	$this->SetY(7);
    $this->Cell(50.1);
    $this->Cell(149,21,'BALANCE DE PRUEBA',0,1,'C'); 
	...
	if(isset($_POST[cc]) ){
		if(!empty($_POST[cc])){
			$sqlr="select nombre from centrocosto where estado='S' AND id_cc='$_POST[cc]' ";
			$res=mysql_query($sqlr,$linkbd);
			$row = mysql_fetch_row($res);
			$this->Cell(135,5,'Centro de Costo: '.$row[0],0,0,'C');
		}
	}
