...
<html xmlns="http://www.w3.org/1999/xhtml"> 
	...
	<body id="cuerpomensaje" class="cuerpomensaje">
        ...
        <form name ="client" id="client" method="POST" >
        	...
        	<table  class="inicio">
            	<tr>
                	<td colspan="5" class="titulos" style="width:85%;" >:.Ingresar Alerta Diaria </td>
            	...
            	<tr>
                    ...
                    <td style="width:30%"><input type="date" name="fechaevento" id="fechaevento" value="<?php echo $_POST[fechaevento] ?>" style="width:90%" min="<?php echo $_POST[fechahoy]?>"  onChange="document.client.submit()"></td>
              	</tr>
                <tr>
			...
			<?php
                ...
                {
                    $linkbd=conectar_bd();
                    $sqlr="SELECT MAX(codigo) FROM alertasdiarias";
                    $resp = mysql_query($sqlr,$linkbd);
                    $row =mysql_fetch_row($resp);
                    $mxa=$row[0];
                    $mxa++;
                    $sqlr = "INSERT INTO alertasdiarias (codigo,usuario,fechaevento,fechainicio,frecuencia,descripcion, fechaverificacion,estado) VALUES ($mxa,'$_SESSION[cedulausu]','$_POST[fechaevento]','".$_POST[rfechas]." ".$_POST[rhoras].":00', '$_POST[frecuenciah]','$_POST[desevento]','','A')";
                    if(mysql_query($sqlr,$linkbd))
                    {
                         echo "<div class='renglon'>Se ha Guardado</div>";	
                        ?><script>parent.document.form2.submit();parent.despliegamodal2("hidden");</script><?php					 
                    }
