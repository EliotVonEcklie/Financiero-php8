...
class PDF extends FPDF
{
	...
	function Footer()
	{
		...
		$this->ln(5);
		$this->Cell(0,10,utf8_decode('Página Web: '.strtolower($datBasicos[3])),0,0,'C');
...
for($v=0;$v<$disc;$v++)
{
	...
	if($dcodcatas[$v]!=$actual)
	{
		...
		$tercero="";
		$ntercero="";
		$sqlr2="select tl.fecha, tp.* from tesopredios AS tp, tesocobroreporte AS tl where tp.cedulacatastral='".$dcodcatas[$v]."' and tp.cedulacatastral=tl.codcatastral and tp.estado='S' $crit3 ";
		...
		while($row2=mysql_fetch_row($res2))
		{
			...
			$m2=$row2[10];
			$ac=$row2[11];
			$zona=obtenerNombreZona($row2[16],0);
 		}
		...
		$age = substr($fecha,0,4);
		$posy=$pdf->GetY();
		$pdf->SetY($posy+10);
		$pdf->Cell(0.5);
		$pdf->SetFont('Arial','B',10);		
		$pdf->MultiCell(195,7,'FACTURA No. '.$numresolucion[$v].' - '.$age,0,'C'); //'.$numresolucion[$v].'
		$pdf->MultiCell(195,8,utf8_decode('DETERMINACION DEL IMPUESTO PREDIAL UNIFICADO - ACTO DE LIQUIDACION OFICIAL SECRETARIA ADMINISTRATIVA Y FINANCIERA '.strtoupper($rs)),0,'C');
		$pdf->SetFont('Arial','',10);
		$pdf->MultiCell(195,4,utf8_decode('La Secretaria administrativa y financiera '.$rs.' , en uso de sus facultades legales y especialmente las conferidas en el Art.66 Ley 383 de 1997; Art.59 Ley 788 de 2002; Acuerdo 016 del 31 de Agosto del 2017, Estatuto Tributario Municipal profiere el presente acto de liquidación del Impuesto Predial Unificado, tasas y sobretasas, con relación al predio identificado con cedula catastral No.'.$dcodcatas[$v].' denominado y/o ubicado en '.strtoupper($direccion).' zona '.$zona.', del '.$rs.', y con cargo al contribuyente '.$ntercero.' identificado con la C.C./Nit. No.'.$tercero.' y/o actual propietario, tenedor y/o poseedor del predio referido, deuda que equivale a la suma de '.convertir($totdeuda).' PESOS ($ '.number_format($totdeuda).'.oo), discriminados en los siguientes de los siguientes periodos gravables y conceptos que permiten calcular el monto de la obligación, así:'),0,'J');	
		...
		$pdf->Cell(10,4,utf8_decode('TARIFA'),0,0,'C');
		...
		$pdf->RoundedRect(10, 129, 199,6.5, 0.5, '1111', '');
		...
		$pdf->SetFont('Arial','',10);
		$pdf->MultiCell(195,4,utf8_decode('Contra la presente Liquidación Oficial procede el Recurso de Reconsideración, el cual deberá interponerse ante el Alcalde Municipal, dentro de los dos (2) meses siguientes a la notificación del presente acto, en virtud de los Artículos 384 y siguientes del Acuerdo Municipal No.016 de 2017.Una vez en firme el presente acto administrativo presta merito ejecutivo.'),0,'J');
		$pdf->SetFont('Arial','B',10);
		$pdf->MultiCell(195,8,utf8_decode('CONSTANCIA DE NOTIFICACION'),0,'C');
		$pdf->SetFont('Arial','',10);
		$pdf->MultiCell(195,4,utf8_decode('La notificación de la factura se realizará mediante inserción en la página web de la Alcaldía del '.$rs.' '.strtolower($datBasicos[0]).' y, simultáneamente con la publicación en medios físicos en el registro, cartelera o lugar visible de la Secretaria de Hacienda Municipal- Alcaldía conforme al artículo No.354 Ley 1819 de 2016 y articulo No.47 Acuerdo Municipal No.016 de 2017.'),0,'J');
		...
		$mes = (int) $mes;
		$pdf->MultiCell(195,4,utf8_decode('Dada en '.$rs.' a los '.strtoupper(convertir($dias)).' ('.$dias.') días del mes de '.$meses[$mes-1].' de '.strtoupper(convertir($age)).' ('.$age.').'),0,'J');
		$pdf->SetFont('Arial','B',10);		
		$pdf->MultiCell(200,6," \n \n NOTIFIQUESE Y CUMPLASE  \n ",0,'C');	
		$pdf->SetFont('Times','B',9);
		$pdf->ln(10);
		$pdf->MultiCell(200,6," \n ".utf8_decode(strtoupper($teso))." \n SECRETARIA ADMINISTRATIVA Y FINANCIERA",0,'C');	
		$posy3=$pdf->GetY();
		$pdf->Image('imagenes/firma_jein.JPG' , 75 ,$posy3-34, 70 ,20,'jpg');
		$pdf->ln(4);
		$pdf->SetFont('times','',8);
