<?php 
	...
	$objPHPExcel->getActiveSheet()->getStyle('A3:Q3')->applyFromArray($borders);
	...
	while ($row =mysql_fetch_assoc($resp)) 
	{
		...
		$sq="select id_recibos,estado from tesoreciboscaja where tipo='1' and id_recaudo='".$row['idpredial']."'".$scrit;
		...
		$re2 = mysql_query($sq2,$linkbd);
		$row2 =mysql_fetch_array($re2);
		if($row1[0]!='')
		{
			$tasa=$row['predial']*1000/$row2[0];
			if(round($tasa)=='')
			{
				$tasa=$row['tasav'];
			}
			$objWorksheet->fromArray(array($row['idpredial'],$row1[0],$row['vigliquidada'],utf8_encode("COD:".$row['codigocatastral']),$row['tercero']." - ".$tercero,$row['fecha'],$row2[0],round($tasa),$row['predial'],$row['intpredial'],$row['bomberil'],$row['intbomb'],$row['medioambiente'],$row['intmedioambiente'],$row['descuentos'],$row['totaliquidavig'],$row1[1]),NULL,"A$cont");
			$objPHPExcel->getActiveSheet()->getStyle("A$cont:Q$cont")->applyFromArray($borders);
			$objPHPExcel->getActiveSheet()->getStyle("H$cont:I$cont")->getNumberFormat()
			->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
			$objPHPExcel->getActiveSheet()->getStyle('A3:Q3')->getAlignment()->applyFromArray(array('vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER,));
			$objPHPExcel->getActiveSheet()->getStyle('A4:Q4')->getAlignment()->applyFromArray(array('vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER,));
			...
			$objPHPExcel->getActiveSheet()->getStyle('A:Q')->getAlignment()->applyFromArray(array('vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER,));
