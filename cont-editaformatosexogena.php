...
<table>
	...
	<tr>
		<td colspan="3" class="cinta">
			<a href="cont-formatosexogena.php" ><img src="imagenes/add.png" class="mgbt title="Nuevo" /></a>
			<a href="#" onClick="guardar()"><img src="imagenes/guarda.png" class="mgbt title="Guardar" /></a>
			<a href="cont-buscaformatosexogena.php" > <img src="imagenes/busca.png" class="mgbt title="Buscar" /></a>
...
$vigusu=vigencia_usuarios($_SESSION[cedulausu]); 
   ...
   $check1='checked';
    ...
    $_POST[dtipos]=array();	
	//*****datos basicos *******
	$sqlr="select *from contexogenaforma_cab where codigo='$_GET[cod]'";
	...
	while($row=mysql_fetch_row($res))
 	...
 	//*****datos terceros *******
	$sqlr="select *from contexogenaforma_basicos where codigo='$_GET[cod]'";
	$res=mysql_query($sqlr,$linkbd);
	while($row=mysql_fetch_row($res))
	 {
		$_POST[listadatos][]=$row[1];
	 }

	//*****campos *******
	$sqlr="select *from contexogenaforma_campos where codigo='$_GET[cod]'";
	$res=mysql_query($sqlr,$linkbd);
	while($row=mysql_fetch_row($res))
	 {
		$_POST[idcampos][]=$row[1];
    	$_POST[dcampos][]=$row[2];
	    $_POST[dvalores][]=$row[3];	
	 }
	//*****conceptos *******
	$sqlr="select *from contexogenaforma_det where codigo='$_GET[cod]'";
	$res=mysql_query($sqlr,$linkbd);
	while($row=mysql_fetch_row($res))
	 {
		$_POST[dcuentas][]=$row[1];
		$sqlr="select *from contexogenaconce_cab where codigo='$row[1]' ";
		$resc=mysql_query($sqlr,$linkbd);
		while ($rowc =mysql_fetch_row($resc)) 				    
    	$_POST[dncuentas][]=$rowc[1];
	 }
	//*****CONCEPTOS - CAMPOS *******
	$sqlr="select *from contexogenaforma_campos_conce where codigo='$_GET[cod]'";
	$res=mysql_query($sqlr,$linkbd);
	while($row=mysql_fetch_row($res))
	 {
		$_POST[camposvar][]=$row[1];
    	$_POST[didconc][]=$row[2];
	    $_POST[didcuentas][]=$row[3];	
	 }
...
<div class="tabs_full">
   <div class="tab">
	   ...
	   <label for="tab-1">Formatos Exogena - Datos Basicos</label>
	   <div class="content">
     <table class="inicio">  
     <tr >
        <td class="titulos"  colspan="6">:: Formatos Exogena</td><td width="5%" class="cerrar" ><a href="cont-principal.php">Cerrar</a></td>
      </tr>     
      <tr  >
	...
	onKeyUp="return tabular(event,this)" value="<?php echo $_POST[nombre]?>" onClick="document.getElementById('nombre').focus();document.getElementById('nombre').select();"></td>
       ...
       <table class="iniciop">
       <tr><td class="titulos" colspan="3">Datos Basicos</td></tr>
  ...
  <div class="tab">
	   ...
	   <label for="tab-2">Formatos Exogena - Campos</label>
	   <div class="content">
      <table class="inicio">  
      <tr >
...
//			  	 echo "bcr--".$_POST[bcr];
 ...
 <div class="tab">
	   ...
	   <label for="tab-3">Formatos Exogena - Agregar Concepto</label>
	   <div class="content">
      <table class="inicio">  
      <tr >
        ...
        <tr  >
        <td  class="saludo1">:: Conceptos:
       </td>
        <td ><select name="concepto"  onChange="validar()" onKeyUp="return tabular(event,this)">
	...
	?>
   </select><input type="hidden" id="nconcepto" name="nconcepto" size="10" value="<?php echo $_POST[nconcepto]?>"></td><td> <input type="button" name="agrega2" value="  Agregar  " onClick="agregardetalle2()" ><input type="hidden" value="0" name="agregadet2"><input name="oculto2" type="hidden" id="oculto2" value="1" ></td>
        </tr>   
        </table>
        <table class="inicio">
        <tr><td class="titulos" colspan="3">Detalle Conceptos Formato</td></tr>
        <tr><td class="titulos2">Codigo Cocnepto</td><td class="titulos2">Nombre Concepto</td><td class="titulos2"><center><img src="imagenes/del.png"><input type='hidden' name='elimina2' id='elimina2'></center></td></tr>   
        <?php
		// echo "bcr".$_POST[bcr];
		//  echo "bcre".$_POST[bcre];
		// echo "bct".$_POST[bct];
...
//			  	 echo "bcr--".$_POST[bcr];
 ...
 <div class="tab">
	   ...
	   <label for="tab-4">Formatos Exogena - Variables</label>
	   <div class="content">
       <table class="inicio">  
      <tr >
...
<?php	
$linkbd=conectar_bd();
$sqlr="update contexogenaforma_cab set nombre='".$_POST[nombre]."' where codigo='".$_POST[codigo]."'";
if (mysql_query($sqlr,$linkbd))
	{
	$numctas=count($_POST[dcampos]);
	$sqlr="delete from contexogenaforma_campos where codigo='".$_POST[codigo]."'";
...
// echo "<div><div>sqlr:".$sqlr."</div></div>";
	...
	$sqlr="delete from contexogenaforma_basicos where codigo='".$_POST[codigo]."'";
	mysql_query($sqlr,$linkbd);
		//echo "numero ".$numctas;
...
// echo "<div><div>sqlr:".$sqlr."</div></div>";
	...
	//************ ALMACENA CONCEPTOS ************
	$sqlr="delete from contexogenaforma_det where codigo='".$_POST[codigo]."'";
	mysql_query($sqlr,$linkbd);
	$numctas=count($_POST[dcuentas]);
	for ($x=0;$x<$numctas;$x++)
 	{
// echo "<div><div>sqlr:".$sqlr."</div></div>";
	...
	$sqlr="delete from contexogenaforma_campos_conce where codigo='".$_POST[codigo]."'";
	mysql_query($sqlr,$linkbd);
