...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		function despliegamodal2(_valor,_tip)
			...
			function guardar()
			{
				var libre=document.getElementById("eslibre").value;
				if(libre=='1'){
					despliegamodalm4('visible','2','El proyecto ha sido utilizado');
				}else{
					if (document.form2.codigo.value!='' && document.form2.nombre.value!='' && document.form2.valorproyecto.value!='' && document.form2.nomarchadj.value!='')
			  	{
			...
			function atrasc(){
				var consec=parseInt(document.getElementById("consecutivo").value);
				
				var min=1;
				if(consec>1){
					document.getElementById("consecutivo").value=parseInt(document.getElementById("consecutivo").value)-1;
					document.getElementById("eslibre").value='';
			...
			{
				var coding=document.getElementById("consecutivo").value;
	...
	<body>
		...
		<table>
        	...
        	<tr>
  				<td colspan="3" class="cinta"><a href="plan-proyectos.php" class="mgbt"><img src="imagenes/add.png" title="Nuevo" border="0" /></a><a href="#" onClick="guardar()" class="mgbt"><img src="imagenes/guarda.png" title="Guardar"/></a><a href="plan-proyectosbuscar.php" class="mgbt"><img src="imagenes/busca.png" title="Buscar" border="0"/></a><a href="#" onClick="mypop=window.open('plan-principal.php','',''); mypop.focus();" class="mgbt"><img src="imagenes/nv.png" title="Nueva Ventana"></a><img  src="imagenes/iratras.png" title="Atr&aacute;s" onClick="location.href='plan-proyectosbuscar.php'" class="mgbt"/></td>
			</tr>
     	</table>
 		<form name="form2" method="post" enctype="multipart/form-data" action="plan-proyectoseditar.php">
		<?php
				...
				if(isset($_GET[idproceso])){
					$_POST[consecutivo]=$_GET[idproceso];
					}
				$sql="SELECT * FROM planproyectos ";
				$result=mysql_query($sql,$linkbd);
				$nummax=mysql_num_rows($result);
				$_POST[maximo]=$nummax;
				
				
				
				$sql="SELECT * FROM planproyectos WHERE consecutivo=$_POST[consecutivo]";
				$result=mysql_query($sql,$linkbd);
				...
				$sql="SELECT MAX(cod_meta) FROM planproyectos_det WHERE codigo='$_POST[codigo]' AND vigencia=$_POST[vigencia] ";
				$result=mysql_query($sql,$linkbd);
				...
				$sql="SELECT valor,nombre_valor,cod_meta FROM planproyectos_det WHERE codigo='$_POST[codigo]' AND vigencia=$_POST[vigencia] ORDER BY LENGTH(valor),cod_meta ASC";
				$result=mysql_query($sql,$linkbd);
				//for($j=0;$j<$_POST[contador]; $j++){
                ...
                // }
                 //-----
                $sql="SELECT * FROM planproyectos_adj WHERE codigo='$_POST[codigo]' AND vigencia=$_POST[vigencia]";
				$result=mysql_query($sql,$linkbd);
               	while($row = mysql_fetch_row($result)){
               	...
               	$sql="SELECT estado FROM planproyectos_pres WHERE codigo='$_POST[codigo]' AND vigencia=$_POST[vigencia]";
               	$result=mysql_query($sql,$linkbd);
               	$row = mysql_fetch_row($result);
               	if($row[0]=="C"){
					$_POST[onoffswitch]==1;
               		$sql="SELECT ppp.cuenta,ppp.valor,pc.nombre,ppp.vigencia FROM planproyectos_pres AS ppp ,pptocuentas AS pc WHERE ppp.codigo='$_POST[codigo]' AND ppp.vigencia=$_POST[vigencia] AND ppp.cuenta=pc.cuenta AND ppp.vigencia=pc.vigencia";
					$result=mysql_query($sql,$linkbd);
					...
					$_POST[onoffswitch]==0;
               		$sql="SELECT ppp.valor,(SELECT nombre FROM (SELECT codigo,nombre FROM pptofutfuentefunc UNION SELECT codigo,nombre FROM pptofutfuentefunc) AS tabla WHERE codigo=ppp.fuente) AS celda,ppp.fuente FROM planproyectos_pres AS ppp WHERE ppp.codigo='$_POST[codigo]' AND ppp.vigencia=$_POST[vigencia] ";
					$result=mysql_query($sql,$linkbd);
					while($rowp = mysql_fetch_row($result)){
			...
			$sql="SELECT * FROM contrasolicitudproyecto WHERE codproyecto='$_POST[codigo]' AND estado='CE' ";
        ...
        <div class="tabsmeci"  style="height:76.5%; width:99.6%">
                    ...
                    <div class="content" style="overflow:hidden;">
                        <table class="inicio" >
                            ...
                            <tr>
                                ...
                                <td style="width:42.5%" colspan="4"><input type="text" name="nomarchadj" id="nomarchadj"  style="width:95%;text-align: right" value="<?php echo $_POST[nomarchadj]?>" readonly><img src="imagenes/descargar.png"  title="descargar" onClick="redireccion()" style="cursor:pointer !important"/></td>
                ...
                <div class="tab">
                    ...
                    <div class="content" style="overflow:hidden;" >
						...
						<input type="hidden" name="consecutivo" id="consecutivo" value="<?php echo $_POST[consecutivo]; ?>">
						<input type="hidden" name="maximo" id="maximo" value="<?php echo $_POST[maximo]; ?>">
						<input type="hidden" name="eslibre" id="eslibre" value="<?php echo $_POST[eslibre]; ?>">
						<?php
                            if($_POST[onoffswitch]==1)
                            {
                                echo"
 		...
 		<?php  
		 	...
		 	if($_POST[oculto]=="2")
			{
				...
				$linkbd=conectar_bd();
				//----Elimina registros
				$sql="SELECT consecutivo FROM planproyectos WHERE codigo='$_POST[codigo]'";
				$res=mysql_query($sql,$linkbd);
				$cons=mysql_fetch_row($res);
				$consec=$cons[0];
				
				$sql="DELETE FROM planproyectos WHERE codigo='$_POST[codigo]'";
				mysql_query($sql,$linkbd);
				$sql="DELETE FROM planproyectos_det WHERE codigo='$_POST[codigo]' ";
				mysql_query($sql,$linkbd);
				$sql="DELETE FROM planproyectos_adj WHERE codigo='$_POST[codigo]' ";
				mysql_query($sql,$linkbd);
				$sql="DELETE FROM planproyectos_pres WHERE codigo='$_POST[codigo]' ";
				mysql_query($sql,$linkbd);
				...
				if (!mysql_query($sqlr,$linkbd))
				{
					...
					{ 
						$tamCuentas=count($_POST[dcuentas]);
						$tamFuentes=count($_POST[fuentesfinan]);
							if($_POST[contador]==0){
