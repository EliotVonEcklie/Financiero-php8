...
//V 1000 12/12/16  
	...
	class MYPDF extends TCPDF 
	{
		public function Header() 
		{
			...
			$this->SetY(10);
			$this->SetX(62);
			$this->Cell(100,17,"ACTO ADMINISTRATIVO DE ACUERDO DE DONACIÓN",1,0,'C'); 
...
EOD;
	...
	$pdf->SetTitle('Donaciones');
	$pdf->SetSubject('Actos Administrativos de Donaciones');
	...
	$pdf->Cell(30,4,"Señor(es):",0,1,'L',false,0,0,false,'T','C');
	$pdf->Cell(190,4,iconv($_SESSION["VERCARPDFINI"], $_SESSION["VERCARPDFFIN"]."//TRANSLIT",$_POST[ntercero]),0,1,'L',false,0,0,false,'T','C');
	$pdf->Cell(190,4,iconv($_SESSION["VERCARPDFINI"], $_SESSION["VERCARPDFFIN"]."//TRANSLIT",$_POST[lugarfi]),0,1,'L',false,0,0,false,'T','C');
	$pdf->Cell(190,4,iconv($_SESSION["VERCARPDFINI"], $_SESSION["VERCARPDFFIN"]."//TRANSLIT",$_POST[ciudad]),0,1,'L',false,0,0,false,'T','C');
	$pdf->ln(8);
	$sqlr="SELECT representante,cedulareplegal FROM configbasica WHERE estado='S'";
	$resp=mysql_query($sqlr,$linkbd);
	$row=mysql_fetch_row($resp);
	$responsable=iconv($_SESSION["VERCARPDFINI"], $_SESSION["VERCARPDFFIN"]."//TRANSLIT",$row[0]);
	$infoinicio="Quién suscribe la presente, señor(a): $responsabl, Documento de Identidad Nº: $row[1] certifica que la ".iconv($_SESSION["VERCARPDFINI"], $_SESSION["VERCARPDFFIN"]."//TRANSLIT",$_POST[ndonante])." expresa la voluntad de donar ".iconv($_SESSION["VERCARPDFINI"], $_SESSION["VERCARPDFFIN"]."//TRANSLIT",$_POST[motivo]).", de acuerdo al detalle siguiente:";
	...
	$pdf->Cell(35,4,"Código",1,0,'C',false,0,0,false,'T','C');
	$pdf->Cell(52,4,"Articulo",1,0,'C',false,0,0,false,'T','C');
	$pdf->Cell(35,4,"Unidad de Medida",1,0,'C',false,0,0,false,'T','C');
	$pdf->Cell(30,4,"Cantidad",1,0,'C',false,0,0,false,'T','C');
	$pdf->Cell(30,4,"Estado",1,1,'C',false,0,0,false,'T','C');
	$con=0;
	while ($con<count($_POST[agarticulo]))
	{
		$altura=6;
		$altini=6;
		$ancini=50;
		$altaux=0;
		$colst01=strlen(iconv($_SESSION["VERCARPDFINI"], $_SESSION["VERCARPDFFIN"]."//TRANSLIT",$_POST[agnarticulo][$con]));
		if($colst01 > $ancini)
		{
		...
		$pdf->Cell(35,$altura,$_POST[agarticulo][$con],1,0,'L',true,0,0,false,'T','C');
		$pdf->MultiCell(52,$altura,iconv($_SESSION["VERCARPDFINI"], $_SESSION["VERCARPDFFIN"]."//TRANSLIT",$_POST[agnarticulo][$con]),1,'L',true,0,'','',true,0,false,true,$altura,'M',false);
		$pdf->Cell(35,$altura,iconv($_SESSION["VERCARPDFINI"], $_SESSION["VERCARPDFFIN"]."//TRANSLIT",$_POST[agunimedida][$con]),1,0,'L',true,0,0,false,'T','C');
	...
	$pdf->ln(4);
	$pdf->SetFont('helvetica','',12);
	$valorletras= convertir($_POST[valort]);
	$infoadicional="El valor estimado total de la donación asciende a: $valorletras  PESOS M/CTE. ($_POST[valort])";
	...
	$sqlr="SELECT nombrecargo FROM planaccargos WHERE codcargo='1'";
	$resp=mysql_query($sqlr,$linkbd);
	$row=mysql_fetch_row($resp);
	$pdf->SetFont('helvetica','',10);
	$pdf->SetX(55);
	$pdf->Cell(88,4,$responsable,'T',1,'C',false,0,0,false,'T','C');
	$pdf->SetFont('helvetica','',9);
	$pdf->SetX(55);
	$pdf->Cell(88,4,iconv($_SESSION["VERCARPDFINI"], $_SESSION["VERCARPDFFIN"]."//TRANSLIT",$row[0]),0,1,'C',false,0,0,false,'T','C');
	$pdf->ln(15);
	$con=0;
	$conx=10;
	$cony=$pdf->GetY();
	...
	// ---------------------------------------------------------
	$pdf->Output('Actodonacionesentrada.pdf', 'I');//Close and output PDF document
?>