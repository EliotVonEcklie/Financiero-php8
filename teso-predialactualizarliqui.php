		...
		<script>
			function adelante(scrtop, numpag, limreg, filtro, next){
				...
				if(parseFloat(maximo)>parseFloat(actual)){
					...
					document.form2.action="teso-predialactualizarliqui.php?idcta="+idcta+"&scrtop="+scrtop+"&numpag="+numpag+"&limreg="+limreg+"&filtro="+filtro;
					document.form2.submit();
				}
			}
			...
			function iratras(scrtop, numpag, limreg, filtro){
				var idcta=document.getElementById('numpredial').value;
				location.href="teso-buscaactualizapredial.php?idcta="+idcta+"&scrtop="+scrtop+"&numpag="+numpag+"&limreg="+limreg+"&filtro="+filtro;
			}

			function guardar(){
				var existeVacio=false;
				vtotalpred=document.getElementsByName("dpredial[]"); 	
				vtotaliqui=document.getElementsByName("dhavaluos[]"); 	
				vtotalbomb=document.getElementsByName("dimpuesto1[]"); 	
				vtotalmedio=document.getElementsByName("dimpuesto2[]"); 	
				...
				vtotalintma=document.getElementsByName("dinteres2[]"); 	
				vtotaldes=document.getElementsByName("ddescuentos[]"); 
				vtotaltasa=document.getElementsByName("tasas[]");
				for(var i=0;i<vtotaliqui.length;i++){
					if(vtotalpred.item(i).value==""){
						existeVacio=true;
						break;
					}
					if(vtotaliqui.item(i).value==""){
						existeVacio=true;
						break;
					}
					if(vtotalbomb.item(i).value==""){
						existeVacio=true;
						break;
					}
					if(vtotalmedio.item(i).value==""){
						existeVacio=true;
						break;
					}
					if(vtotalintp.item(i).value==""){
						existeVacio=true;
						break;
					}
					if(vtotalintb.item(i).value==""){
						existeVacio=true;
						break;
					}
					if(vtotalintma.item(i).value==""){
						existeVacio=true;
						break;
					}
					if(vtotaldes.item(i).value==""){
						existeVacio=true;
						break;
					}
					if(vtotaltasa.item(i).value==""){
						existeVacio=true;
						break;
					}
				...
				if(existeVacio==true){
					alert("No pueden existir valores vacios");
				}else{
					if(confirm("Realmente desea guardar dicha actualizacion")){
  ...
  <?php
	...
	while($row=mysql_fetch_row($res)){
		...
		if($_POST[tipop]=='urbano'){
		 ...
		 $_POST[buscav]="";
		 $sqlr2="select IF(tasa='-1',0,tasa) from tesoprediosavaluos where vigencia='".$_SESSION[vigencia]."' AND codigocatastral='$row[0]' ";
	...
	<div class="tabspre">
      ...
      <table class="inicio">
		...
		<tr>
		  ...
		  <td class="titulos2">Codigo Catastral</td>
   		  ...
   		  <td class="titulos2">Tasa Predial</td>
		  ...
		  <td width="3%" class="titulos2">Sel
			...
			//*************	
			 echo "<tr><td class='saludo1'><input name='dvigencias[]' value='".$r3[1]."' type='text' size='4' readonly></td><td class='saludo1'><input name='dcodcatas[]' value='".$_POST[codcat]."' type='text' size='16' readonly><input name='dvaloravaluo[]' value='".$r3[2]."' type='hidden' ><input name='dtasavig[]' value='".$r3[3]."' type='hidden' ></td><td class='saludo1'><input name='dpredial[]' value='".$r3[4]."' type='text' size='12' ></td><td class='saludo1'><input name='tasas[]' value='".$r3[3]."' type='text' size='12' ></td><td class='saludo1'><input name='dipredial[]' value='".$r3[5]."' type='text' size='7' ></td><td class='saludo1'><input name='dimpuesto1[]' value='".$r3[6]."' type='text'  size='12' ></td><td class='saludo1'><input name='dinteres1[]' value='".$r3[7]."' type='text' size='7' ></td><td class='saludo1'><input name='dimpuesto2[]' value='".$r3[8]."' type='text'  size='12' ></td><td class='saludo1'><input name='dinteres2[]' value='".$r3[9]."' type='text' size='7' ></td><td class='saludo1'><input type='text' name='ddescuentos[]' value='$r3[10]' size='6' ></td><td class='saludo1'><input name='davaluos[]' value='".$r3[11]."' type='text' size='10' ><input name='dhavaluos[]' value='".$r3[11]."' type='hidden' ></td><td class='saludo1'><input type='text' name='dias[]' value='$diasd' size='4' readonly></td><td class='saludo1'><input type='hidden' name='dselvigencias[]' value='$r3[1]' onClick='buscavigencias(this)' >Ok</td></tr>";
		 $_POST[totalc]=$_POST[totalc]+$r3[11];
      ...
      <?php
      	if($_POST[oculto]=="2"){
      		...
      		$totpredial=array_sum($_POST[dpredial]);
      		$totipredial=array_sum($_POST[dipredial]);
      		$totbomb=array_sum($_POST[dimpuesto1]);
      		$totibomb=array_sum($_POST[dinteres1]);
      		$totamb=array_sum($_POST[dimpuesto2]);
      		$totiamb=array_sum($_POST[dinteres2]);
      		$totdesc=array_sum($_POST[ddescuentos]);
      		$totvaltot=array_sum($_POST[davaluos]);
      		$totintereses=$totipredial+$totibomb+$totiamb;
      		try{
      			$sql="UPDATE tesoliquidapredial SET totaliquida='$totvaltot',totalpredial='$totpredial',totalbomb='$totbomb',totalmedio='$totamb',totalinteres='$totintereses',intpredial='$totipredial',intbomb='$totibomb',intmedio='$totiamb',totaldescuentos='$totdesc' WHERE codigocatastral='$_POST[codcat]' AND idpredial='$_POST[numpredial]' ";
      		//echo $sql;
      		mysql_query($sql,$linkbd);
      		$sql="UPDATE tesoreciboscaja SET valor='$totvaltot' WHERE id_recaudo='$_POST[numpredial]' ";
      		mysql_query($sql,$linkbd);
      		$sql="SELECT id_recibos  FROM tesoreciboscaja WHERE id_recaudo='$_POST[numpredial]'";
      		$res=mysql_query($sql,$linkbd);
      		$fila=mysql_fetch_row($res);
      		$numrecibo=$fila[0];
      		$sql="DELETE FROM tesoreciboscaja_det WHERE id_recibos='$numrecibo'";
      		mysql_query($sql,$linkbd);
      		for($i=0;$i<count($_POST[dpredial]); $i++){
      			$sqlr="SELECT tasav,predial,intpredial,bomberil,intbomb,medioambiente,intmedioambiente,descuentos,totaliquidavig FROM tesoliquidapredial_det WHERE idpredial='$_POST[numpredial]' AND vigliquidada='".$_POST[dvigencias][$i]."' ";
      			$result=mysql_query($sqlr,$linkbd);
      			$registros=mysql_fetch_row($result);

      			$sql="INSERT INTO tesoauditapredial(liquidacion,vigencia,predialant,predialnu,tasaant,tasanu,ipredialant,ipredialnu,bomberosant,bomberosnu,ibomberosant,ibomberosnu,ambienteant,ambientenu,iambienteant,iambientenu,descuentoant,descuentonu,valtotalant,valtotalnu,usuario) VALUES ('$_POST[numpredial]','".$_POST[dvigencias][$i]."','$registros[1]','".$_POST[dpredial][$i]."','$registros[0]','".$_POST[tasas][$i]."','$registros[2]','".$_POST[dipredial][$i]."','$registros[3]','".$_POST[dimpuesto1][$i]."','$registros[4]','".$_POST[dinteres1][$i]."','$registros[5]','".$_POST[dimpuesto2][$i]."','$registros[6]','".$_POST[dinteres2][$i]."','$registros[7]','".$_POST[ddescuentos][$i]."','$registros[8]','".$_POST[davaluos][$i]."','".$_SESSION["nickusu"]."')";

      			mysql_query($sql,$linkbd);
      			$sqlr="UPDATE tesoliquidapredial_det SET tasav='".$_POST[tasas][$i]."',predial='".$_POST[dpredial][$i]."',intpredial='".$_POST[dipredial][$i]."',bomberil='".$_POST[dimpuesto1][$i]."',intbomb='".$_POST[dinteres1][$i]."',medioambiente='".$_POST[dimpuesto2][$i]."',intmedioambiente='".$_POST[dinteres2][$i]."',descuentos='".$_POST[ddescuentos][$i]."',totaliquidavig='".$_POST[davaluos][$i]."' WHERE idpredial='$_POST[numpredial]' AND vigliquidada='".$_POST[dvigencias][$i]."' ";
      			mysql_query($sqlr,$linkbd);
      			
      			if($i==count($_POST[dpredial])-1){
      				$sqlr2="UPDATE tesoliquidapredial SET tasapredial='$_POST[tasas][$i]' WHERE codigocatastral='$_POST[codcat]' AND idpredial='$_POST[numpredial]' ";
      				mysql_query($sqlr2,$linkbd);
      				$sqlr="INSERT INTO tesoreciboscaja_det (id_recibos,ingreso,valor,estado) VALUES ('$numrecibo','01','".$_POST[davaluos][$i]."','S') ";
      				mysql_query($sqlr,$linkbd);
      			}else{
      				$sqlr="INSERT INTO tesoreciboscaja_det (id_recibos,ingreso,valor,estado) VALUES ('$numrecibo','03','".$_POST[davaluos][$i]."','S') ";
      				mysql_query($sqlr,$linkbd);
      			}
      		}
      		echo "<script>document.getElementById('oculto').value='3';</script>";
      		$ruta="http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
      		echo "<meta http-equiv='refresh' content='0;url=$ruta'>";
      		echo "<script>alert('Liquidacion actualizada con exito');</script>";
      		}catch(Exception $e){
