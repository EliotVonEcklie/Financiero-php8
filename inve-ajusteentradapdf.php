...
//V 1000 12/12/16  
	...
	class MYPDF extends TCPDF 
	{
		public function Header() 
		{
			...
			$this->SetY(10);
			$this->SetX(62);
			$this->Cell(100,17,"ACTO ADMINISTRATIVO ".$_POST[tipomov],1,0,'C'); 
...
EOD;
	...
	$pdf->SetSubject('Actos Administrativos de Ajustes');
	...
	$infoinicio="En la ciudad de $_POST[ciudad] a los $fechaen[0] días del mes de $mesen del $fechaen[2] en el $_POST[lugarfi], se procede a la entrada por ajuste de los bienes, efectuada por $_POST[ntercero].";
	...
	$pdf->Cell(30,4,"Articulos Ajustados:",0,1,'L',false,0,0,false,'T','C');
	$pdf->Cell(8,4,"N°",1,0,'C',false,0,0,false,'T','C');
	$pdf->Cell(56,4,"Descripción",1,0,'C',false,0,0,false,'T','C');
	$pdf->Cell(32,4,"Unidad de Medida",1,0,'C',false,0,0,false,'T','C');
	$pdf->Cell(18,4,"Cantidad",1,0,'C',false,0,0,false,'T','C');
	$pdf->Cell(26,4,"Valor Unitario",1,0,'C',false,0,0,false,'T','C');
	$pdf->Cell(28,4,"Valor Total",1,0,'C',false,0,0,false,'T','C');
	$pdf->Cell(26,4,"Estado",1,1,'C',false,0,0,false,'T','C');
	$con=0;
	while ($con<count($_POST[agdescripcion]))
	{
		...
		$pdf->MultiCell(56,$altura,iconv($_SESSION["VERCARPDFINI"], $_SESSION["VERCARPDFFIN"]."//TRANSLIT",$_POST[agdescripcion][$con]),1,'L',true,0,'','',true,0,false,true,$altura,'M',false);
		$pdf->Cell(32,$altura,iconv($_SESSION["VERCARPDFINI"], $_SESSION["VERCARPDFFIN"]."//TRANSLIT",$_POST[agunimedida][$con]),1,0,'L',true,0,0,false,'T','C');
		$pdf->Cell(18,$altura,number_format($_POST[agcantidad][$con],0,$_SESSION["spdecimal"],$_SESSION["spmillares"])."   ",1,0,'R',true,0,0,false,'T','C');
		$pdf->Cell(26,$altura,"$ ".number_format($_POST[agvalor][$con],2,$_SESSION["spdecimal"],$_SESSION["spmillares"])."   ",1,0,'R',true,0,0,false,'T','C');
		$total = $_POST[agvalor][$con]*$_POST[agcantidad][$con];
		$pdf->Cell(28,$altura,"$ ".number_format($total,2,$_SESSION["spdecimal"],$_SESSION["spmillares"])."   ",1,0,'R',true,0,0,false,'T','C');
		$pdf->Cell(26,$altura,$mdestado,1,1,'L',true,0,0,false,'T','C');
		$totalc += $total;
		$con++;
	}
	$pdf->Cell(130,6,'TOTAL:',0,0,'R',false,0,0,false,'T','C');
	$pdf->setFont('times','B',9);
	$pdf->Cell(38,6,"$ ".number_format($totalc,2,$_SESSION["spdecimal"],$_SESSION["spmillares"])."  ",1,1,'R',false,0,0,false,'T','C');
	$pdf->ln(2);
	$pdf->SetFont('helvetica','',12);
	$infoadicional="Al respecto, se hace entrega en fotocopia de la documentación sustentatoria presentada; sobre la legalidad de los bienes, para su registro respectivo.";
	...
	// ---------------------------------------------------------
	$pdf->Output('Actoajustesentrada.pdf', 'I');//Close and output PDF document
?>