...
//   $niv=$_SESSION["nivel"];
  ...
  //echo "Quepaso".$sqlr;
    ...
    {
	 $_SESSION[linksette][$roww[2]].='<li> <a onClick="location.href=\''.$roww[1].'\'" style="cursor:pointer;">'.$roww[0].'  <span style="float:right">'.$roww[3].'</span></a></li>';
	}  
 }
}
else
...
<span id="todastablas2"></span>
  ...
  <table width="50%"  class="tablaprin" align="center"   >
        <tr>
          ...
          <?php
		   $fecha=date("d/m/Y");
		  ereg( "([0-9]{1,2})/([0-9]{1,2})/([0-9]{2,4})", $fecha,$fechas);
	$fechaf=$fechas[3]."-".$fechas[2]."-".$fechas[1];
	     //anularprediales($fechaf);
          ?>
          </td>
        </tr>
      </table>  
