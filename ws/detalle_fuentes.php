...
$tabla='<table width="100%" class="titulos2" >
	<tr >
		...
		<td align="center" style="width: 15% ">'.$vigenciai.' ($)</td>
		<td align="center" style="width: 15% ">'.($vigenciai+1).' ($)</td>
		<td align="center" style="width: 15% ">'.($vigenciai+2).' ($)</td>
		<td align="center" style="width: 15% ">'.($vigenciai+3).' ($)</td>
	</tr>';
	$j=$vigenciai;
	for($i=$vigenciai;$i<=$vigenciaf;$i++){
		$arreglo=Array();
		$sql1="SELECT TB4.cuentap,TB4.valor,TB3.vigencia FROM presucdpplandesarrollo AS TB1,pptorp AS TB2,tesoordenpago AS TB3,pptocdp AS TB5,tesoordenpago_det AS TB4 WHERE TB1.codigo_meta='$meta' AND TB1.id_cdp=TB5.consvigencia AND  TB5.tipo_mov='201' AND TB2.idcdp=TB1.id_cdp AND TB1.vigencia='$i' AND TB2.tipo_mov='201' AND TB2.vigencia='$i' AND TB3.id_rp=TB2.consvigencia AND TB3.tipo_mov='201' AND TB3.id_orden=TB4.id_orden AND TB3.vigencia='$i' AND NOT EXISTS (SELECT 1 FROM tesoegresos AS TEGRE WHERE TEGRE.tipo_mov='401' AND TEGRE.id_orden=TB3.id_orden) ";
		...
		while($row=mysql_fetch_array($result)){
			...
			$tabla.='<td class="cssdeta">$ '.number_format($arreglo[$j],2,',','.').'</td><td class="cssdeta">$ '.number_format($arreglo[$j+1],2,',','.').'</td><td class="cssdeta">$ '.number_format($arreglo[$j+2],2,',','.').'</td><td class="cssdeta">$ '.number_format($arreglo[$j+3],2,',','.').'</td>';
