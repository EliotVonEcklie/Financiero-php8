...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="es">
		...
		<script>
			function verUltimaPos(idcta, filas, filtro){
				...
				if((limreg==0)||(limreg==""))
					limreg=10;
				numpag++;
				location.href="meci-editaanexos.php?idproceso="+idcta+"&scrtop="+scrtop+"&totreg="+filas+"&altura="+altura+"&numpag="+numpag+"&limreg="+limreg+"&filtro="+filtro;
			}
		</script>
		<script>
	...
	<body>
		...
		<table>
			...
			<tr>
  				<td colspan="3" class="cinta">
					<a onclick="location.href='meci-anexos.php'" class="tooltip bottom mgbt"><img src="imagenes/add.png" /><span class="tiptext">Nuevo</span></a>
					<a class="tooltip bottom mgbt"><img src="imagenes/guardad.png"/></a>
					<a onClick="document.form2.submit()" class="tooltip bottom mgbt"><img src="imagenes/busca.png" border="0" /><span class="tiptext">Buscar</span></a>
		...
		?>
 		<form name="form2" method="post" action="meci-buscaanexos.php">
        	<?php
				if($_POST[oculto]=="")
				{
					$_POST[cambioestado]="";
				...
				if($_POST[cambioestado]!="")
				{
					...
					{
						$sqlr="UPDATE calanexos SET estado='S' WHERE id='$_POST[idestado]'";
						mysql_fetch_row(mysql_query($sqlr,$linkbd)); 
					}
					else 
					{
						$sqlr="UPDATE calanexos SET estado='N' WHERE id='$_POST[idestado]'";
						mysql_fetch_row(mysql_query($sqlr,$linkbd)); 
					}
				}
				//*****************************************************************
			...
			<table  class="inicio ancho" align="center">
                <tr>
                    <td class="titulos" colspan="2">:: Buscar Anexos </td>
    		...
    		<div class="subpantallac5" style="height:66%; width:99.6%; overflow-x:hidden;" id="divdet">
      			<?php
					...
					$sqlr="SELECT * FROM calanexos WHERE estado <> '' $crit1";
					...
					$sqlr="SELECT * FROM calanexos WHERE estado <> '' $crit1 ORDER BY id ".$cond2;
					$resp = mysql_query($sqlr,$linkbd);
					$con=1;
					...
					<table class='inicio' align='center' width='80%'>
						<tr>
							...
							<td class='submenu'>
                                <select name='renumres' id='renumres' onChange='cambionum();'>
						...
						<tr class='centrartext'>
							...
							<td class='titulos2' width='90%'>Nombre</td>
					...
					$filas=1;
 					while ($row =mysql_fetch_row($resp)) 
 					{
						...
						echo"<tr class='$iter' onDblClick=\"verUltimaPos($idcta, $numfil, $filtro)\" style='text-transform:uppercase; $estilo' >
							...
							<td><input type='range' name='lswitch1[]' value='".$_POST[lswitch1][$row[0]]."' min ='0' max='1' step ='1' style='background:$coloracti; width:90%' onChange='cambioswitch(\"$row[0]\",\"".$_POST[lswitch1][$row[0]]."\")' /></td>
							</tr>";
						$con+=1;
						$aux=$iter;
						$iter=$iter2;
