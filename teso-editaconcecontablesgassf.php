		...
		<script>
			function adelante(scrtop, numpag, limreg, filtro, next){
				...
				if(parseFloat(maximo)>parseFloat(actual)){
					...
					document.form2.action="teso-editaconcecontablesgassf.php?idcta="+idcta+"&scrtop="+scrtop+"&numpag="+numpag+"&limreg="+limreg+"&filtro="+filtro;
					document.form2.submit();
				}
			}
			...
			function iratras(scrtop, numpag, limreg, filtro){
				var idcta=document.getElementById('numero').value;
				location.href="teso-buscaconcecontablesgassf.php?idcta="+idcta+"&scrtop="+scrtop+"&numpag="+numpag+"&limreg="+limreg+"&filtro="+filtro;
			}
			function validar3(formulario)
			{
				document.form2.action="teso-editaconcecontablescausa.php";
				document.form2.submit();
			}
...
<table>
	...
	<tr>
		<td colspan="3" class="cinta">
			<a href="teso-concecontablesgassf.php" class="mgbt"><img src="imagenes/add.png" title="Nuevo" border="0" /></a>
			<a href="#" onClick="guardar()" class="mgbt"><img src="imagenes/guarda.png" title="Guardar" /></a>
			<a href="teso-buscaconcecontablesgassf.php" class="mgbt"><img src="imagenes/busca.png" title="Buscar" border="0" /></a>
...
<?php
		...
		$sqlr="select MIN(CONVERT(codigo, SIGNED INTEGER)), MAX(CONVERT(codigo, SIGNED INTEGER)) from conceptoscontables where modulo='4' and tipo='GS' ORDER BY CONVERT(codigo, SIGNED INTEGER)";
		...
		if($_POST[oculto]==""){
			if ($_POST[codrec]!="" || $_GET[is]!=""){
				if($_POST[codrec]!=""){
					$sqlr="select *from conceptoscontables where codigo='$_POST[codrec]' and modulo='4' and tipo='GS'";
				}
				else{
					$sqlr="select *from conceptoscontables where codigo ='$_GET[is]' and modulo='4' and tipo='GS'";
				}
			}
			else{
				$sqlr="select * from  conceptoscontables where modulo='4' and tipo='GS' ORDER BY CONVERT(codigo, SIGNED INTEGER) DESC";
			}
			$res=mysql_query($sqlr,$linkbd);
		...
		if(($_POST[oculto]!="2")&&($_POST[oculto]!="9")){
			$linkbd=conectar_bd();
			$sqlr="select *from conceptoscontables where conceptoscontables.codigo=$_POST[numero] and conceptoscontables.tipo='GS'  and conceptoscontables.modulo=4";
			$res=mysql_query($sqlr,$linkbd); 
		...
		if(($_POST[oculto]!="2")&&($_POST[oculto]!="9")){
			...
			unset($_POST[cuenta]);
			unset($_POST[ncuenta]);	
			$sqlr="select *from conceptoscontables, conceptoscontables_det  where conceptoscontables.codigo=conceptoscontables_det.codigo and conceptoscontables.codigo=$_POST[numero] and conceptoscontables.tipo='GS' and conceptoscontables_det.tipo='GS' and conceptoscontables.modulo='4' and conceptoscontables_det.modulo='4' and conceptoscontables_det.cuenta!='' order by conceptoscontables_det.fechainicial desc";
			$res=mysql_query($sqlr,$linkbd); 
			$cont=0;
		...
		//NEXT
		$sqln="select *from conceptoscontables WHERE codigo > '$_POST[numero]' and modulo='4' and tipo='GS' ORDER BY codigo ASC LIMIT 1";
		$resn=mysql_query($sqln,$linkbd);
		$row=mysql_fetch_row($resn);
		$next="'".$row[0]."'";
		//PREV
		$sqlp="select *from conceptoscontables WHERE codigo < '$_POST[numero]' and modulo='4' and tipo='GS' ORDER BY codigo DESC LIMIT 1";
...
<?php //**** busca cuenta
    ...
    <table class="inicio" align="center"  >
      	<tr >
        	<td class="titulos" colspan="8">.: Editar Concepto Contable Gastos SSF </td>
		  ...
		  onKeyUp="return tabular(event,this)" value="<?php echo $_POST[numero]?>" onClick="document.getElementById('numero').focus();document.getElementById('numero').select();">
		 	...
		 	<td style="width:2.5cm" class="saludo1">Nombre:</td>
          	...
          	<td style="width:15%">
                ...
                <input name="tipoc" value="GS" size="10" type="hidden"><input name="ban" type="hidden"  value="1">
           	</td>
	    </tr>
    </table>
	<table class="inicio">
	...
	if ($_POST[agregadet]=='1')
		 ...
		 {echo "<tr class='$iter' onMouseOver=\"anterior=this.style.backgroundColor;this.style.backgroundColor='#40b3ff';\" onMouseOut=\"this.style.backgroundColor=anterior\">
			...
			<td>
				<input name='dccs[]' value='".$_POST[dccs][$x]."' type='text' size='2' class='inpnovisibles' readonly>
			</td>
			<td>
