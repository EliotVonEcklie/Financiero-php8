...
document.form2.action="teso-egreso-regrabarppto.php";
document.form2.submit();
}
}
</script>
...
if($_POST[oculto]=='2')
{
	...
	$fechaf=$fecha[3]."-".$fecha[2]."-".$fecha[1];
	$sqlr="delete from pptocomprobante_cab  where numerotipo=$_POST[idcomp] and tipo_comp=8";
	mysql_query($sqlr,$linkbd);
//	$sqlr="update comprobante_det set tercero='$_POST[tercero]' where id_comp='11 $_POST[idcomp]'";
	$sqlr="delete from pptocomprobante_det where numerotipo=$_POST[idcomp] and tipo_comp=8";
	mysql_query($sqlr,$linkbd);

	 $sqlr="insert into  pptocomprobante_cab (numerotipo,tipo_comp,fecha,concepto,vigencia,total_debito,total_credito,diferencia,estado) values($_POST[idcomp],8,'$fechaf','CXP $_POST[detallegreso]',$_POST[vigencia],0,0,0,'$estado')";	 
...
//	$_POST[idcomp]=$idcomp;
	...
	//******************* DETALLE DEL COMPROBANTE CONTABLE  *********************
		...
		for($x=0;$x<count($_POST[dcuentas]);$x++)
	 	{
		 //***BUSCAR CUENTA PPTAL ***************
		$sqlr="insert into  pptocomprobante_det (cuenta,tercero,detalle,valdebito,valcredito,estado,vigencia,tipo_comp,numerotipo) values('".$_POST[dcuentas][$x]."','".$_POST[tercero]."','CxP ',".$_POST[dvalores][$x].",0,'$estado','$_POST[vigencia]',8,'$_POST[idcomp]')";
		mysql_query($sqlr,$linkbd); 					
		//echo "<br>".$sqlr;
	  	}
