...
class PDF extends FPDF
{
	...
	{	
			...
			$this->SetY(8);
		    $this->Cell(50.1);
		    $this->Cell(149,20,'DETERMINACION IMPUESTO PREDIAL UNIFICADO',0,1,'C'); 
			$this->SetFont('Arial','B',9);
		    $this->Cell(250,-10,utf8_decode('LIQUIDACION OFICIAL - SECRETARIA DE HACIENDA'),0,0,'C'); 			
			//************************************
		    $this->SetFont('Arial','B',10);
	...
	function Footer()
	{
		...
		$sql="SELECT web,direccion,telefono,email FROM configbasica";
		$res=mysql_query($sql,$linkbd);
		$datBasicos=mysql_fetch_row($res);
	    $this->SetY(-17);
		$this->SetFont('Arial','I',10);
		$this->Cell(0,10,' URL '.$datBasicos[0].' / e-mail: '.$datBasicos[3],0,0,'C'); 
		$this->ln(5);
...
for($v=0;$v<$disc;$v++)
{
	...
	if($dcodcatas[$v]!=$actual)
	{
		...
		$pdf->SetY(27);
		$pdf->Cell(50.2);
		$pdf->multiCell(110.7,7,'FACTURA No '.$numresolucion[$v],'T','R');
			
		$posy=$pdf->GetY();
    	$pdf->SetY($posy+10);
		$pdf->Cell(1);
		$pdf->SetFont('Times','',10);
		$pdf->MultiCell(195,4,utf8_decode("Por medio de la presente liquidación oficial se determina una obligación a favor del Municipio de Puerto Rico por valor de ($ ".number_format($totdeuda,2,',','.').") por concepto de Impuesto Predial Unificado, Sobretasa Bomberil y Sobretasa Ambiental más intereses a cargo del propietario o poseedor actual del predio con Cedula Catastral No. ".$dcodcatas[$v].", ubicado en la siguiente dirección ".$direccion.", que aparece actualmente en la base catastral del Instituto Geografico Agustin Codazzi como propiedad de: ".$ntercero.", en uso de las facultades legales y que le confiere el Art. 415 del Estatuto Rentas Municipal, acuerdo 026 de diciembre de 2016 al suscrito secretario de Hacienda Municipal, así:"),0,'J');		
		$pdf->Cell(1);
		$pdf->SetFont('Times','B',10);
		...
		*/
		...
		$pdf->SetFont('Times','',10);
		//$pdf->MultiCell(195,4,utf8_decode("\nSEGUNDO: Por lo anterior, solicito a usted que dentro de los diez (10) días siguientes a la fecha de la presente liquidación, se acerque a este Despacho a efectuar el pago correspondiente o a suscribir acuerdo de pago, en caso contrario la administración Municipal se ve obligada a iniciar proceso de ejecución coactiva.\nTERCERO: Contra la presente resolución procede el recurso de reconsideración, el cual deberá interponerse ante la secretaria de Hacienda Municipal dentro de los dos(2) meses siguientes a su notificación. CUARTO: Una vez ejecutoriada la presente resolución, constituirá titulo ejecutivo en contra del sujeto pasivo del impuesto predial unificado, conforme a lo dispuesto en el numeral primero de la parte resolutiva de la presente resolución y en contra de sus herederos o legatarios. QUINTO: NOTIFICASE de conformidad con lo dispuesto en los artículos 431 y subsiguientes del Estatuto Tributario Municipal."),0,'J');
		$posy=$pdf->GetY();
		$pdf->SetY($posy+5);
		$pdf->MultiCell(194,4,utf8_decode("Que la administración Municipal estableció los plazos para pagar las vigencias antes señaladas, y el contribuyente incumplió con su obligación tributaria de hacerlo.
		"),0,'J');
		$pdf->MultiCell(195,4,utf8_decode('En contra de la presente liquidación procede el recurso de reconsideración, el cual deberá interponerse ante la secretaria de Hacienda Municipal dentro de los dos (2) meses siguientes a su notificación y publicación, en virtud del Art. 720 del Estatuto Tributario Nacional.
		'),0,'J');	
		...
		$pdf->SetFont('Times','',10);
		$pdf->MultiCell(195,4,utf8_decode("La notificación de la factura se realizará mediante inserción en la página web de la ".strtolower($rs)." ".$datBasicos[0]." y simultáneamente con la publicación en medios físicos en el registro, cartelera o hacienda - Alcaldía conforme al Art. No. 354 ley 1819 de 2016 y Art. No. 5 el Acuerdo 020 de 2017.
		"),0,'J');
		$pdf->MultiCell(195,4,utf8_decode("Lo anterior dado en ".strtolower($rs)." a los ".strtoupper(convertir(date('d')))." (".(date('d')).") días del mes de ".(strftime('%B'))." de ".strtoupper(convertir(date('Y')))." (".(date('Y')).")."),0,'J');
		$pdf->SetFont('Times','B',10);
		...
		$posy3=$pdf->GetY();
		$pdf->Image('imagenes/firma_hernan.jpg' , 75 ,$posy3-40, 50 ,30,'jpg');
	$nr=$nr+1;
	}//fin de if
