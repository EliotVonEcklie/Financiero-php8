...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		function compruebabanco(verifica,index){
			...
			if(bloqueo!="1"){
				...
				document.getElementById("aceptab[" + index + "]").value = "0";
				}else{
					document.getElementById("contadorcert").value=parseInt(document.getElementById("contadorcert").value)-1;
					document.getElementById("aceptab[" + index + "]").value = "1";
				}
			}
		...
		if(id=='finaliza'){
			 if(cantidad==0){
				 despliegamodalm4('visible','4','Debe certificar por lo menos una meta');
				 document.form2.finaliza.checked=false;
			 }else if(totalAporte > valorTotalProyecto){
				 despliegamodalm4('visible','4','El total de aportes debe ser menor que el valor del proyecto');
				 document.form2.finaliza.checked=false;
			 }else{
				 if(check){
		...
		function despliegamodal2(_valor,_tip)
			...
			function guardar()
			{
				var liberado=document.getElementById("esliberado").value;
				var aporteMunicipio = document.form2.aporconvu.value;
				var aporteConvenio = document.form2.apormuniu.value;
				var valorTotalProyecto = document.form2.valorproyectosinformato.value;
				...
				if(liberado=='1'){
					despliegamodalm4('visible','2','El certificado ya ha sido utilizado');
				}else{
					
					if(totalAporte <= valorTotalProyecto){
						if (document.form2.solproyecod.value!='' && document.form2.fecha.value!='' && document.form2.codigoproy.value!='' && document.form2.contador.value>0)
						{
							despliegamodalm('visible','4','Esta Seguro de Guardar');
						}
						else
						{
	...
	<?php
		function existeSolicitudMeta($proyecto,$meta){
			global $linkbd;
			$sql="SELECT metascert FROM contrasolicitudproyecto where contrasolicitudproyecto.codigo='$proyecto' ";
			$res=mysql_query($sql,$linkbd);
			$row=mysql_fetch_row($res);
			$metascert=explode("-",$row[0]);
			if(in_array($meta,$metascert)){
				$num=1;
			}else{
				$num=0;
			}
		...
		function obtenerCodigoMeta($solicitud,$meta){
			global $linkbd;
			$sql="SELECT codsolicitud FROM contrasolicitudproyecto WHERE codigo='$solicitud' ";
			$res=mysql_query($sql,$linkbd);
			$row=mysql_fetch_row($res);
			$sql="SELECT cod_meta FROM contrasolicitudproyecto_det WHERE codigosol='".$row[0]."' AND valor='$meta'  ";
			$res=mysql_query($sql,$linkbd);
	...
	<body>
		...
		<?php
			...
			if(isset($_GET[id])){
				...
				$sql="SELECT * FROM contrasolicitudproyecto WHERE codigo=$_POST[autoincre]";
				$result=mysql_query($sql,$linkbd);
				while($row1 = mysql_fetch_row($result)){
					
					$sqles="SELECT activo FROM contrasoladquisiciones WHERE codsolicitud='$row1[1]' ";
					$resi=mysql_query($sqles,$linkbd);
		            $fila=mysql_fetch_row($resi);
					$_POST[esliberado]=$fila[0];
					if($_POST[oculto]==""){
						if(!empty($row1[13])){
							$arreglomet=explode("-",$row1[13]);
							$_POST[contadorcert]=count($arreglomet);
						}else{
							$_POST[contadorcert]=0;
						}
						
						$_POST[codigoproy]=$row1[2];
						if($row1[4]=='CE'){
							$_POST[finaliza]="1";
							$_POST[bloqueartodo] = "1";
						}else if($row1[4]=='CO'){
							$_POST[finaliza2]="1";
						}
						$_POST[observa]=$row1[9];
						if(!empty($row1[11])){
							$_POST[fecha]=$row1[11];
						}else{
						...
						$_POST[apormuni]=$row1[8];
						$_POST[aporconv]=$row1[7];
						$sqlpa="SELECT valtotalsol FROM contrasolicitudpaa WHERE codsolicitud='$row1[1]' ";
						$resa=mysql_query($sqlpa,$linkbd);
						$valorpaa=mysql_fetch_row($resa);
						$_POST[valacti]=$valorpaa[0];
					}
					
					$_POST[solproyecod]=$row1[0];
  			...
  			if($_POST[oculto]==""){
				...
				$_POST[onoffswitch]=1;	
				$_POST[tabgroup1]=1;
				$sql="SELECT MAX(contrasolicitudproyecto_det.cod_meta) FROM contrasolicitudproyecto_det,contrasolicitudproyecto WHERE contrasolicitudproyecto_det.codigosol=contrasolicitudproyecto.codsolicitud AND contrasolicitudproyecto.codigo=$_POST[autoincre] ";
				//$sql="SELECT MAX(planproyectos_det.cod_meta) FROM planproyectos,planproyectos_det WHERE planproyectos.codigo=planproyectos_det.codigo AND planproyectos.codigo='$_POST[codigoproy]' AND planproyectos.estado NOT IN ('N','R') ";
				$result=mysql_query($sql,$linkbd);
				...
				$sql="SELECT contrasolicitudproyecto_det.valor,contrasolicitudproyecto_det.nombre_valor,contrasolicitudproyecto_det.cod_meta FROM contrasolicitudproyecto_det,contrasolicitudproyecto WHERE contrasolicitudproyecto_det.codigosol=contrasolicitudproyecto.codsolicitud AND contrasolicitudproyecto.codigo=$_POST[autoincre] ORDER BY LENGTH(contrasolicitudproyecto_det.valor),contrasolicitudproyecto_det.cod_meta ASC";
				//$sql = "SELECT planproyectos_det.valor,planproyectos_det.nombre_valor,planproyectos_det.cod_meta FROM planproyectos,planproyectos_det WHERE planproyectos.codigo=planproyectos_det.codigo AND planproyectos.codigo='$_POST[codigoproy]' AND planproyectos.estado NOT IN ('N','R') ORDER BY LENGTH(planproyectos_det.valor),planproyectos_det.cod_meta ASC";
				$result=mysql_query($sql,$linkbd);
        ...
        <div class="tabsmeci"  style="height:76.5%; width:99.6%">
                <div class="tab">
					...
					<table class="inicio">
						<tr>
							<td class="titulos" style="width:100%;">.: Reversar
								<select name="tipo" id="tipo" onKeyUp="return tabular(event,this)" onChange="cargarproyecto()" style="width:20%;" >
									<option value='1' <?php if($_POST[tipo]==1){ echo SELECTED; }?>>Total</option>
								</select>
							</td>
							<td style="width:80%;">
							</td>
					...
					<table class="inicio" >
                        ...
                        <table class="inicio" >
                            ...
                            <tr>
                                ...
                                <td style="width:20%"><input type="text" name="codigoproy" id="codigoproy" value="<?php echo $_POST[codigoproy]?>" readonly style="width:92%" onKeyUp="return tabular(event,this)" onBlur="buscaproyectos()" onClick="document.getElementById('codigoproy').focus(); document.getElementById('codigoproy').select();" ><a href='#' ><img src='imagenes/find02.png' style='width:20px;cursor:pointer;'/></a></td>
							...
							if(!$_POST[oculto]){
							 ...
							 echo"
                                ...
                                $iter2n='saludo2';
								for($x=0;$x<$_POST[contador]; $x++){
									if(existeSolicitudMeta($_POST[autoincre],$_POST["matmetas$x"][$conta-1])>0 && isset($_POST[aceptab][$x]) || $_POST[bloqueartodo]=="1"){
									...
									for ($y=0;$y<$conta;$y++)
									{
										...
										if($y==0){
												echo "<td><input type='checkbox' name='aceptab[$x]' id='aceptab[$x]'  onChange='compruebabanco(this,$x)' $check $disabled /></td>";
											}
										echo "<td>";
