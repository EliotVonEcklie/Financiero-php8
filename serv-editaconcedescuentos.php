		...
		<script>
			function adelante(scrtop, numpag, limreg, filtro, next){
				...
				if(parseFloat(maximo)>parseFloat(actual)){
					...
					document.form2.action="serv-editaconcedescuentos.php?idcta="+idcta+"&scrtop="+scrtop+"&numpag="+numpag+"&limreg="+limreg+"&filtro="+filtro;
					document.form2.submit();
				}
			}
            ...
            function iratras(scrtop, numpag, limreg, filtro){
                var idcta=document.getElementById('numero').value;
                location.href="serv-buscaconcedescuentos.php?idcta="+idcta+"&scrtop="+scrtop+"&numpag="+numpag+"&limreg="+limreg+"&filtro="+filtro;
            }
        </script>
...
<table>
	...
	<tr>
		<td colspan="3" class="cinta">
			<a href="serv-concedescuentos.php" class="mgbt"><img src="imagenes/add.png"  title="Nuevo" border="0" /></a>
			<a href="#"  onClick="guardar()" class="mgbt"><img src="imagenes/guarda.png"  title="Guardar" /></a>
			<a href="serv-buscaconcedescuentos.php" class="mgbt"><img src="imagenes/busca.png"  title="Buscar" border="0" /></a>
...
<?php
		...
		$sqlr="select MIN(CONVERT(codigo, SIGNED INTEGER)), MAX(CONVERT(codigo, SIGNED INTEGER)) from conceptoscontables where modulo='10' and tipo='SD' ORDER BY CONVERT(codigo, SIGNED INTEGER)";
		...
		if($_POST[oculto]==""){
			if ($_POST[codrec]!="" || $_GET[is]!=""){
				if($_POST[codrec]!=""){
					$sqlr="select *from conceptoscontables where codigo='$_POST[codrec]' and modulo='10' and tipo='SD'";
				}
				else{
					$sqlr="select *from conceptoscontables where codigo ='$_GET[is]' and modulo='10' and tipo='SD'";
				}
			}
			else{
				$sqlr="select * from  conceptoscontables where modulo='10' and tipo='SD' ORDER BY CONVERT(codigo, SIGNED INTEGER) DESC";
			}
			$res=mysql_query($sqlr,$linkbd);
		...
		if(($_POST[oculto]!="2")&&($_POST[oculto]!="9")){
		 	$linkbd=conectar_bd();
 		 	$sqlr="SELECT * FROM conceptoscontables where modulo='10' and tipo='SD' and codigo='$_POST[numero]'";
		...
		if(($_POST[oculto]!="2")&&($_POST[oculto]!="9")){
			...
			unset($_POST[cuenta]);
			unset($_POST[ncuenta]);
 		 	$sqlr="SELECT * FROM conceptoscontables_det where codigo ='$_POST[numero]' and tipo='SD' and tipocuenta='N' and cuenta!='' order by fechainicial desc";
		 	$resp = mysql_query($sqlr,$linkbd);
		 	$cont=0;
		 	$_POST[tipoc]='SD';
		 	while ($row =mysql_fetch_assoc($resp)){
		...
		//NEXT
		$sqln="select *from conceptoscontables WHERE codigo > '$_POST[numero]' and modulo='10' and tipo='SD' ORDER BY codigo ASC LIMIT 1";
		$resn=mysql_query($sqln,$linkbd);
		$row=mysql_fetch_row($resn);
		$next="'".$row[0]."'";
		//PREV
		$sqlp="select *from conceptoscontables WHERE codigo < '$_POST[numero]' and modulo='10' and tipo='SD' ORDER BY codigo DESC LIMIT 1";
...
<?php //**** busca cuenta
	...
	<table class="inicio" align="center"  >
    	<tr>
        	<td class="titulos" colspan="6">.: Editar Concepto Contable Descuentos </td>
          ...
          <td style="width:15%">
          	<input name="tipo" value="Descuentos"  style="width:100%" type="text" readonly>
            <input name="tipoc" value="SD" size="10" type="hidden">
          </td>
	 	</tr>
  	</table>
	<table class="inicio">
