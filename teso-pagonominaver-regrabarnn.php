...
<table>
	<tr><td>
<img src="imagenes/logoteso.png"></td><td><img src="imagenes/entidad.png"></td><td>
		<table class="inicio">
		...
		<td class="saludo1">Perfil: </td><td><?php echo $_SESSION["perfil"];?></td></tr>
		 <tr><td  class="saludo1" >Fecha ingreso:</td><td> <?php echo " ".$fec=date("Y-m-d");?> </td> <td  class="saludo1">Hora Ingreso: </td><td><?php $hora=time();echo " ".date ( "h:i:s" , $hora ); $hora=date ( "h:i:s" , $hora )?></td></tr></table></td></tr>
	<tr><td colspan="3">
        <!-- Navigation -->  
		<ul class="mi-menu">  
        <li><a href="principal.php">Inicio</a></li>  
        <li><a href="#">Archivos Maestros</a><ul><script>document.write('<?php echo $_SESSION[linksette][1];?>')</script></ul></li>
		<li><a href="#">Recaudo</a><ul><script>document.write('<?php echo $_SESSION[linksette][2];?>')</script></ul></li>		
		<li><a href="#">Pago</a><ul><script>document.write('<?php echo $_SESSION[linksette][3];?>')</script></ul></li>				
		<li><a href="#">Traslados</a><ul><script>document.write('<?php echo $_SESSION[linksette][4];?>')</script></ul></li>						
		<li><a href="#" >Herramientas</a><ul><script>document.write('<?php echo $_SESSION[linksette][5];?>')</script></ul></li>
        <li><a href="#" >Informes</a><ul><script>document.write('<?php echo $_SESSION[linksette][6];?>')</script></ul></li>         
...
$nreg=$row[0];
	...
	//	  echo $sqlr;
		  ...
		  //$sqlr="select *from tesoordenpago_det where id_orden=$_POST[orden] ";
			...
			/*	$sqlr="UPDATE  pptocuentaspptoinicial SET pagos=pagos+".$_POST[devalores][$y]." where vigencia=".$vigusu." and cuenta='".$_POST[derecursos][$y]."'";				
				mysql_query($sqlr,$linkbd);*/
				//$sqlr="UPDATE  pptocuentaspptoinicial SET cxp=cxp-".$_POST[devalores][$y]." where vigencia=".$vigusu." and cuenta='".$_POST[derecursos][$y]."'";				
				//mysql_query($sqlr,$linkbd);
		//		$sqlr="insert into tesoegresosnomina_det (`id_egreso`, `id_orden`, `tipo`, `tercero`, `ntercero_det`, `cuentap`, `cc`, `valor`, `estado`) values ($ideg,$_POST[orden],'".$_POST[tedet][$y]."','$_POST[tercero]','".$_POST[dencuentas][$y]."','".$_POST[derecursos][$y]."','".$_POST[deccs][$y]."','".$_POST[devalores][$y]."','S')";
			//	mysql_query($sqlr,$linkbd);
				 }
		  //***********crear el contabilidad
		 ...
		 // $sqlr="select *from tesoordenpago_det where id_orden=$_POST[orden] ";
			//	$resp2 = mysql_query($sqlr,$linkbd);
				...
				for($y=0;$y<count($_POST[decuentas]);$y++)				
				 {
					if($_POST[tedet][$y]!='D' and $_POST[tedet][$y]!='SE' and $_POST[tedet][$y]!='PE' and $_POST[tedet][$y]!='N')
					//if($_POST[tedet][$y]=='N')
					 { 					 
					 					 
					 $concepto=buscapptovarnom($_POST[derecursos][$y], $_POST[tedet][$y],$vigusu); 					 
					/*$sqlr="select codconcepago from pptocuentas where cuenta='".$_POST[derecursos][$y]."' and (vigencia='".$vigusu."' OR vigenciaf='$vigusu')";	  
					$resp = mysql_query($sqlr,$linkbd);					
					while($rowp=mysql_fetch_row($resp))
					 {*/
					// echo "<br>concepto:".$concepto;
					// $concecontable=buscaparanom($concepto);
					//	$sqlr="select * from conceptoscontables_det where codigo='$rowp[0]' and modulo='3' and cc='".$_POST[deccs][$y]."' and tipo='P'";	  
				...
				while ($rowr=mysql_fetch_row($respc)) 
					{						
					 ...
					 if('S'==$rowr[7])
						...
						$credito=0;
						$ncppto=buscacuentapres($_POST[decuentas][$y],2);
	 					$sqlr="insert into comprobante_det (id_comp,cuenta,tercero,centrocosto,detalle,cheque,valdebito, valcredito, estado, vigencia) values ('17 $ideg','".$rowr[4]."','".$_POST[tercero]."' ,'".$_POST[deccs][$y]."' , 'Pago ".$ncppto."','',".$_POST[devalores][$y].",0,'1' ,'".$vigusu."')";
					mysql_query($sqlr,$linkbd);
				 // echo "<br>".$sqlret;
						 }
				 	 if('S'==$rowr[6])
...
//				echo $sqlr;
		...
		echo "<tr><td  class='saludo2'><input type='text' size='1' name='tedet[]' value='".$row2[3]."' readonly></td><td class='saludo2'><input name='decuentas[]' value='".$row2[4]."' type='text' size='20' readonly><input name='ddescuentos[]' value='".$_POST[ddescuentos][$jp]."' type='hidden'><input type='hidden' name='idedescuento[]' value='".$row2[10]."'></td>";
		 echo "<td class='saludo2'><input name='dencuentas[]' value='".$tercero.' '.$row2[5]."' type='text' size='90' readonly></td>";
		 echo "<td class='saludo2'><input name='deccs[]' value='".$row2[7]."' type='text' size='2' readonly></td>";
		 echo "<td class='saludo2'><input name='derecursos[]' value='".$row2[6]."' type='text' readonly></td>";
		 echo "<td class='saludo2'><input name='devalores[]' value='".$row2[8]."' type='text' size='15' readonly></td></tr>";
