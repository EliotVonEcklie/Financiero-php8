...
{	if ($con%2==0)
    ...
    $pdf->Cell(30,4,''.$_POST[fecrec][$con],0,0,'L',1);//descrip
    $pdf->Cell(50,4,substr(''.$_POST[tiporec][$con],0,50),0,0,'L',1);//descrip
	$pdf->Cell(20,4,''.$_POST[nrec][$con],0,0,'L',1);//descrip
//	$pdf->Cell(90,4,''.buscatercero($_POST[terrec][$con]),0,0,'L',1);//descrip
	$pdf->Cell(140,4,''.substr($_POST[terrec][$con],0,90),0,0,'L',1);//descrip
    $pdf->Cell(40,4,''.number_format($_POST[valrec][$con],2,',','.'),0,0,'R',1);//descrip
	$pdf->ln(4);	
	$con=$con+1;
	$ct=$ct+1;
	if ($ct%$control==0)
...
//$pdf->Cell(180,5,'TOTAL:','T',0,'R');
//$pdf->Cell(80,5,''.number_format(array_sum($_POST[valrec]),2,".",","),'T',0,'R');
$pdf->Output();
?> 
