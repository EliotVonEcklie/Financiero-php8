...
<tr>
  ...
  <a href="teso-reportecobropredial.php" class="mgbt" ><img src="imagenes/add.png" alt="Nuevo"  border="0" title="Nuevo"/></a> 
  <a href="#" onClick="guardar()" class="mgbt"><img src="imagenes/guarda.png"  alt="Guardar" title="Guardar"/></a>
  <a href="teso-buscareportecobropredial.php" class="mgbt"> <img src="imagenes/busca.png"  alt="Buscar" title="Buscar"/></a> 
  ...
  <a href="#" <?php if($_POST[oculto]!="") { ?> onClick="pdf()" <?php } ?> class="mgbt"> <img src="imagenes/print.png"  alt="Buscar"  title="Imprimir"/></a>
...
?>
 <form name="form2" method="post" action="">
  <?php
  $linkbd=conectar_bd();
$sqlr="Select max(nresol) from tesocobropredial";
$res=mysql_query($sqlr,$linkbd);
    ...
    <td class="saludo1">Descuento:</td><td><input name="descuento" type="text" value="<?php echo $_POST[descuento]?>"  size="5" onKeyPress="javascript:return solonumeros(event)" onKeyUp="return tabular(event,this)" readonly>%</td ></tr>
	  ...
	  <td width="128" class="saludo1">Codigo Catastral:</td>
          <td  >
			...
			<a href="#" onClick="mypop=window.open('catastral-ventana4.php','','menubar=0,scrollbars=yes, toolbar=no, location=no, width=900,height=500px');mypop.focus();"><img src="imagenes/buscarep.png" align="absmiddle" border="0"></a>
			<input type="hidden" name="chacuerdo" value="1">
			<input type="hidden" value="1" name="oculto"></td><td>
			<input type="hidden" value="<?php echo $_POST[buscav]?>" name="buscav"><input type="button" name="buscarb" id="buscarb" value=" Ver Periodos " onClick="buscar()" >
			</td>
        </tr>
        <tr><td class="saludo1">Avaluo Vigente:</td><td><input name="avaluo2" value="<?php echo $_POST[avaluo2]?>" type="text" size="20" readonly><input type="hidden" name="vavaluo"  value="<?php echo $_POST[vavaluo]?>" > </td><td class="saludo1">Tasa Predial	:</td><td><input name="tasa" value="<?php echo $_POST[tasa]?>" type="text" size="4" readonly>xmil</td><td ></td><td><input name="predial" value="<?php echo $_POST[predial]?>" type="hidden"  readonly></td></tr>
	  </table>
	</div> 
	</div>
     <div class="tab">
...
//								 echo $tasaintdiaria."-";
	...
	$fechaf=$fecha[3]."-".$fecha[2]."-".$fecha[1];
	  $linkbd=conectar_bd();
	 $sqlr="insert into tesocobropredial (fecha,codcatastral, tercero, total,estado,tasavig,avaluo) values ('$fechaf','$_POST[catastral]','$_POST[tercero]',$_POST[totliquida],'S','','')";
	if(!mysql_query($sqlr,$linkbd))
	 {
	  echo "<table class='inicio'><tr><td class='saludo1'>No Se ha podido Liquidar el Predial <img src='imagenes\alert.png'></td></tr></table>";  
	 }
	  else
	   {
		 $idp=mysql_insert_id(); 
		 ...
		 for($x=0;$x<count($_POST[dvigencias]);$x++)
		  {
			  ...
			  {
		 $sqlr="insert into tesocobropredial_det (`idpredial`, `vigliquidada`, avaluo,tasav,`predial`, `intpredial`, `bomberil`, `intbomb`, `medioambiente`, `intmedioambiente`, `descuentos`, `totaliquidavig`, `estado`) values ($idp,'".$_POST[dvigencias][$x]."',".$_POST[dvaloravaluo][$x].",".$_POST[dtasavig][$x].",".$_POST[dpredial][$x].",".$_POST[dipredial][$x].",".$_POST[dimpuesto1][$x].",".$_POST[dinteres1][$x].",".$_POST[dimpuesto2][$x].",".$_POST[dinteres2][$x].",".$_POST[ddescuentos][$x].",".$_POST[dhavaluos][$x].",'S')";
