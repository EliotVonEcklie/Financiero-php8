...
function buscar()
 ...
 document.form2.vigencia.value=document.form2.bvigencia.value;
 document.form2.submit();
 }
</script>
...
<table>
	...
	<tr>
		<td colspan="3" class="cinta">
			<a href="teso-tarifaspredial.php" class="mgbt"><img src="imagenes/add.png" alt="Nuevo"  border="0" title="Nuevo"/></a> 
...
function obtenerRangoTipo($codigo,$idrango,$tipo){
	global $linkbd;
	$sql="SELECT nombre,nom_rango,avaluo_fijo,val1,val2 FROM teso_clasificapredios WHERE codigo=$codigo AND consecutivo=$idrango";
	//ECHO $sql."<br>";
...
if($_POST[buscav]=='1')
 {
	 ...
	 $linkbd=conectar_bd();
	 $sqlr="select *from tesotarifaspredial where vigencia=".$_POST[bvigencia]." order by tipo,estratos";
	 
	 $res=mysql_query($sqlr,$linkbd);
	 while($row=mysql_fetch_row($res))
	  {
		 $_POST[dncuentas][]=obtenerRangoTipo($row[1],$row[2],1);	
		 $_POST[dtcuentas][]=obtenerRangoTipo($row[1],$row[2],0);
		 $_POST[dvalores][]=$row[5];
		 $_POST[arreglotipo][]=obtenerRangoTipo($row[1],$row[2],2);
		 $tipoul=obtenerRangoTipo($row[1],$row[2],2);
		 $_POST[arreglorango][]=obtenerRangoTipo($row[1],$row[2],3)." - ".obtenerRangoTipo($row[1],$row[2],4);
		 $_POST[buscav]="";
		 $_POST[dtcuentascod][]=$row[1];
		 $_POST[dcuentas][]=$row[2];
	  }
	  if($tipoul=="AVALUO"){
		  $_POST[avaluoc]="1";
	  }else if($tipoul=="FIJO"){
...
?>
<table class="inicio">
<tr><td class="titulos" colspan="2">Tarifas Vigencias</td></tr>
<tr><td width="8%" class="saludo1">Buscar Vigencia:</td><td width="88%"><input name="bvigencia" type="text" value="<?php echo $_POST[bvigencia] ?>"> <input type="button" name="buscarb" id="buscarb" value="   Buscar   " onClick="buscar()" > <input type="hidden" value="<?php echo $_POST[buscav]?>" name="buscav"></td></tr>
</table>
 <?php if($_POST[bt]=='1')
			 {
            ...
            <td style="width:20%;">
           		<select name="rangos" >
					...
					<?php
						...
						if(isset($_POST[tipop])){
							if(!empty($_POST[tipop])){
								$sql="SELECT id_rango,nom_rango FROM teso_clasificapredios WHERE codigo=$_POST[tipop] and vigencia=".$_POST[vigencia]."";
								$result=mysql_query($sql,$linkbd);
								while($row = mysql_fetch_array($result)){
								echo "<option value='$row[0]'>$row[1]</option>";
								}
	   ...
	   <tr>
		...
		$posi=$_POST[elimina];
		 ...
		 // echo "dc:".$_POST[dcuentas][0];
		 for ($x=0;$x<count($_POST[dncuentas]);$x++)
		 {			 
		  echo "<tr class='saludo2'>
		 		<td style='width:25%;'>
		 			<input name='dcuentas[]' value='".$_POST[dcuentas][$x]."' type='hidden'>
		 			<input name='dncuentas[]' value='".$_POST[dncuentas][$x]."' type='text'  class='inpnovisibles' style='width: 100%' readonly>
		 		</td>
		 		<td style='width:20%;'>
				    <input name='dtcuentascod[]' value='".$_POST[dtcuentascod][$x]."' type='hidden'>
		 			<input name='dtcuentas[]' value='".$_POST[dtcuentas][$x]."' type='text'  class='inpnovisibles' style='width: 100%' readonly>
		 		</td>
				<td style='width:20%;'>
...
if($_POST[oculto]=='2')
{
	...
	if (!mysql_query($sqlr,$linkbd))
	{
	 ...
	 echo "<table class='inicio'><tr class='saludo1'><td><center><font color=blue><img src='imagenes/alert.png'> Manejador de Errores de la Clase BD<br><font size=1></font></font><br><p align=center>No se pudo ejecutar la petici�n: <br><font color=red><b>$sqlr</b></font></p>";
