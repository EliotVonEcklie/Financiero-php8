...
document.form2.action="presu-sinrecibocajasp-reflejar.php";
document.form2.submit();
}
}
</script>
...
//	echo $_POST[tiporec];
	switch($_POST[tiporec]){	  
		case 3:
	  		$sqlr="select *from tesosinrecaudossp where tesosinrecaudossp.id_recaudo=$_POST[idrecaudo] and 3=$_POST[tiporec]";
  		//echo "$sqlr";
  	  $_POST[encontro]="";
	  $res=mysql_query($sqlr,$linkbd);
		while ($row =mysql_fetch_row($res)) 
 		...
 		if($_POST[encontro]=='1'){
  			switch($_POST[tiporec]){	 	 
	  			case 3: ///*****************otros recaudos *******************
  					...
  					$sqlr="select *from tesosinrecaudossp_det where tesosinrecaudossp_det.id_recaudo=$_POST[idrecaudo]  and 3=$_POST[tiporec]";
 //echo $sqlr;
		 			$_POST[dcoding]= array(); 		 
...
if($_POST[oculto]=='2')
{
    ...
    $sqlr="delete from pptocomprobante_cab where numerotipo=$_POST[idcomp] and tipo_comp='18' ";
	  mysql_query($sqlr,$linkbd);
		...
		//echo $sqlr;
		mysql_query($sqlr,$linkbd);
		$sqlr="delete from pptosinrecibocajaspppto where idrecibo=$_POST[idcomp]";
		//echo $sqlr;		
		mysql_query($sqlr,$linkbd);
 	ereg( "([0-9]{1,2})/([0-9]{1,2})/([0-9]{2,4})", $_POST[fecha],$fecha);
	$fechaf=$fecha[3]."-".$fecha[2]."-".$fecha[1];
...
//	echo "<input type='hidden' name='ncomp' value='$idcomp'>";
		  ...
		  $sqlr="delete from pptosinrecibocajaspppto where idrecibo=$consec ";
			 mysql_query($sqlr,$linkbd);	
	...
	for($x=0;$x<count($_POST[dcoding]);$x++)
	 {
	  ...
	  $sqlr="insert into pptosinrecibocajaspppto (cuenta,idrecibo,valor,vigencia) values('$rowi[6]',$consec,$vi,'".$vigusu."')";
	  mysql_query($sqlr,$linkbd);	
