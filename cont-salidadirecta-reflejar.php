...
document.form2.action="cont-salidadirecta-reflejar.php";
document.form2.submit();
}
}
</script>
...
if($_GET[consecutivo]!=""){echo "<script>document.getElementById('codrec').value=$_GET[consecutivo];</script>";}
	if(!$_POST[oculto])
	{
		...
		$fec=date("d/m/Y");	
		$link=conectar_bd();
		$sqlr="select * from almginventario where tipomov='2' and tiporeg='06' ORDER BY consec DESC";
		...
		if ($_POST[codrec]!="" || $_GET[consecutivo]!=""){
			if($_POST[codrec]!=""){
                $sqlr="select * from almginventario WHERE  tipomov='2' and tiporeg='06' and consec='$_POST[codrec]'";
			}
			else{
                $sqlr="select * from almginventario WHERE  tipomov='2' and tiporeg='06' and consec='$_GET[consecutivo]'";
			}
		}
		else{
            $sqlr="select * from almginventario where tipomov='2' and tiporeg='06' ORDER BY consec DESC";
	...
	$sqlr="select * from almginventario where  tipomov='2' and tiporeg='06' and consec=".$_POST[idcomp];
	$res=mysql_query($sqlr,$linkbd);
	$consec=0;
	...
	$linkbd=conectar_bd();
    $linkbd=conectar_bd();
    $sqlr="select *from almginventario where tipomov='2' and tiporeg='06' and almginventario.consec=$_POST[idcomp] ";
    $_POST[encontro]="";
    $res=mysql_query($sqlr,$linkbd);
    while ($row =mysql_fetch_row($res))
    {
...
<input type="hidden" name="cc" id="cc" value="<?php echo $_POST[cc] ?>">
	<table class="inicio" align="center" >
    	<tr >
       		<td class="titulos" colspan="10">Interfaz Salida de almacen Directa</td>
	  	...
	  	<tr>
			...
			<td class="saludo1" width="101">Concepto Contable:</td>
           	<td>
               	<input name="conceptocont" type="text" id="conceptocont" onKeyUp="return tabular(event,this)" value="<?php echo $_POST[conceptocont]?>"  style="width:100%; text-align:right;" readonly >
          	</td>
      	</tr>
  	</table>
   	<div class="subpantalla">
    	<?php 
	   	$linkbd=conectar_bd();
		$sqlr="select *from almginventario_det where tipomov='2' and tiporeg='06' and almginventario_det.codigo=$_POST[idcomp] ";
		//echo $sqlr."<br>";
        ...
        while ($row =mysql_fetch_row($res))
        {
  		...
  		?>
	   	<table class="inicio">
	   		<tr>
            	<td colspan="8" class="titulos">Detalle Gestion Inventario Entrada</td>
          	</tr>                  
			<tr>
			...
			echo "<tr class='$iter' onMouseOver=\"anterior=this.style.backgroundColor; this.style.backgroundColor='#40b3ff';\" onMouseOut=\"this.style.backgroundColor=anterior\">
					...
					<td style='width:10%'>
						<input name='agcuen[]' value='".$_POST[agcuen][$x]."' type='text'>
					</td>
					<td style='width:10%'>
...
if($_POST[oculto]=='2')
{
	...
	if($bloquear=="N")
	{
		...
		if($bloq>=1)
		{
			...
			{
				$sqlr="update comprobante_cab set estado=0 where tipo_comp=65 and numerotipo=$_POST[idcomp]";
				mysql_query($sqlr,$linkbd);			
				$sqlr="update comprobante_det set valcredito=0, valcredito=0 where tipo_comp=65 and numerotipo=$_POST[idcomp]";
				mysql_query($sqlr,$linkbd);		
			}
			else
			{
				$sqlr="delete from comprobante_det where id_comp='65 $_POST[idcomp]'";
				mysql_query($sqlr,$linkbd);
				//echo $_POST[causacion];
                //$sqlr="update comprobante_det set tercero='$_POST[tercero]' where id_comp='11 $_POST[idcomp]'";
                $sqlr="delete from comprobante_cab where tipo_comp=65 and numerotipo=$_POST[idcomp]";
                mysql_query($sqlr,$linkbd);			
                ...
                for($x = 0; $x<count($_POST[codart]); $x++)
                {
                    ...
                    if($_POST[agcuen][$x]=='' || $_POST[agcuen][$x]==0)
                    {
                        $_POST[agcuen][$x]=39;
					}
					$sqlr="UPDATE almginventario_det set concepto='".$_POST[agcuen][$x]."' WHERE codigo='$_POST[idcomp]' AND codart='".$_POST[codart][$x]."' AND tipomov=2";
                	mysql_query($sqlr,$linkbd);	
                    //COMPROBANTE DEBITO
                    $sql="SELECT T3.fechainicial,T3.cuenta,T3.cc FROM conceptoscontables T2 INNER JOIN conceptoscontables_det T3 ON T3.codigo = T2.codigo WHERE T2.almacen='S' AND T3.tipo='C' AND T3.cuenta<>'' AND T3.modulo='3' AND T3.cc='".$_POST[ccd][$x]."' AND T3.fechainicial<'".$fechaf."' AND T2.codigo='".$_POST[agcuen][$x]."' ORDER BY T3.fechainicial DESC LIMIT 1";
					$row = view($sql);
                    $sql="INSERT INTO comprobante_det(id_comp,cuenta,tercero,centrocosto,detalle,valdebito,valcredito,estado,vigencia,tipo_comp,numerotipo,numacti,cantarticulo) VALUES ('65 $_POST[idcomp]','".$row[0][cuenta]."','".$_POST[tercero]."','".$row[0][cc]."','$_POST[concepto]','".$_POST[valtotal][$x]."',0,1,'$_POST[anioact]','65','$_POST[idcomp]','".$_POST[codart][$x]."','".$_POST[cantidad][$x]."')";
                    view($sql);
                    ...
                    $row = view($sql);
                    $sql="INSERT INTO comprobante_det(id_comp,cuenta,tercero,centrocosto,detalle,valdebito,valcredito,estado,vigencia,tipo_comp,numerotipo,numacti,cantarticulo) VALUES ('65 $_POST[idcomp]','".$row[0][cuenta]."','".$_POST[tercero]."','".$row[0][cc]."','$_POST[concepto]',0,'".$_POST[valtotal][$x]."',1,'$_POST[anioact]','65','$_POST[idcomp]','".$_POST[codart][$x]."','".$_POST[cantidad][$x]."')";
                    view($sql);
                }
                $sql="INSERT INTO comprobante_cab(numerotipo,tipo_comp,fecha,concepto,total,total_debito,total_credito,diferencia,estado) VALUES ('$_POST[idcomp]','65','$fechaf','$_POST[concepto]','0','".round($_POST[totalalm])."','".round($_POST[totalalm])."','0','1')";
                view($sql);
				...
				?>
				<script>
					despliegamodalm('visible','1',"Se ha almacenado la salida directa de almacen con Exito");
