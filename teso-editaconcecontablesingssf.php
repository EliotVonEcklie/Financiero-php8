		...
		<script>
			function adelante(scrtop, numpag, limreg, filtro, next){
				...
				if(parseFloat(maximo)>parseFloat(actual)){
					...
					document.form2.action="teso-editaconcecontablesingssf.php?idcta="+idcta+"&scrtop="+scrtop+"&numpag="+numpag+"&limreg="+limreg+"&filtro="+filtro;
					document.form2.submit();
				}
			}
			...
			function iratras(scrtop, numpag, limreg, filtro){
				var idcta=document.getElementById('numero').value;
				location.href="teso-buscaconcecontablesingssf.php?idcta="+idcta+"&scrtop="+scrtop+"&numpag="+numpag+"&limreg="+limreg+"&filtro="+filtro;
			}
			
			function validar3(formulario)
			{
				document.form2.action="teso-editaconcecontablespredial.php";
				document.form2.submit();
			}
...
<table>
	...
	<tr>
		<td colspan="3" class="cinta">
			<a href="teso-concecontablesingssf.php" class="mgbt"><img src="imagenes/add.png" titlet="Nuevo" border="0" /></a>
			<a href="#" onClick="guardar()" class="mgbt"><img src="imagenes/guarda.png" title="Guardar" /></a>
			<a href="teso-buscaconcecontablesingssf.php" class="mgbt"><img src="imagenes/busca.png" title="Buscar" border="0" /></a>
			<a onClick="mypop=window.open('plan-agenda.php','','');mypop.focus()" class="mgbt"><img src="imagenes/agenda1.png" title="Agenda" /></a>
			<a href="#" class="mgbt" onClick="mypop=window.open('teso-principal.php','','');mypop.focus();"><img src="imagenes/nv.png" alt="Nueva Ventana"></a>
...
<?php
		...
		$sqlr="select MIN(CONVERT(codigo, SIGNED INTEGER)), MAX(CONVERT(codigo, SIGNED INTEGER)) from conceptoscontables where modulo='4' and tipo='IS' ORDER BY CONVERT(codigo, SIGNED INTEGER)";
		...
		if($_POST[oculto]==""){
			if ($_POST[codrec]!="" || $_GET[is]!=""){
				if($_POST[codrec]!=""){
					$sqlr="select *from conceptoscontables where codigo='$_POST[codrec]' and modulo='4' and tipo='IS'";
				}
				else{
					$sqlr="select *from conceptoscontables where codigo ='$_GET[is]' and modulo='4' and tipo='IS'";
				}
			}
			else{
				$sqlr="select * from  conceptoscontables where modulo='4' and tipo='IS' ORDER BY CONVERT(codigo, SIGNED INTEGER) DESC";
			}
			$res=mysql_query($sqlr,$linkbd);
		...
		if(($_POST[oculto]!="2")&&($_POST[oculto]!="9")){
			$linkbd=conectar_bd();
			$sqlr="select *from conceptoscontables where conceptoscontables.codigo=$_POST[numero] and conceptoscontables.tipo='IS'  and conceptoscontables.modulo=4";
			$res=mysql_query($sqlr,$linkbd); 
		...
		if(($_POST[oculto]!="2")&&($_POST[oculto]!="9")){
			...
			unset($_POST[cuenta]);
			unset($_POST[ncuenta]);			
			$sqlr="select *from conceptoscontables, conceptoscontables_det  where conceptoscontables.codigo=conceptoscontables_det.codigo and conceptoscontables.codigo=$_POST[numero] and conceptoscontables.tipo='IS' and conceptoscontables_det.tipo='IS' and conceptoscontables.modulo='4' and conceptoscontables_det.modulo='4' and conceptoscontables_det.cuenta!='' order by conceptoscontables_det.fechainicial desc";
			//echo $sqlr;
			$res=mysql_query($sqlr,$linkbd); 
			$cont=0;
		...
		//NEXT
		$sqln="select *from conceptoscontables WHERE codigo > '$_POST[numero]' and modulo='4' and tipo='IS' ORDER BY codigo ASC LIMIT 1";
		$resn=mysql_query($sqln,$linkbd);
		$row=mysql_fetch_row($resn);
		$next="'".$row[0]."'";
		//PREV
		$sqlp="select *from conceptoscontables WHERE codigo < '$_POST[numero]' and modulo='4' and tipo='IS' ORDER BY codigo DESC LIMIT 1";
...
<?php //**** busca cuenta
    ...
    <table class="inicio" align="center"  >
      	<tr >
        	<td class="titulos" colspan="8">.: Editar Concepto Contable Ingreso SSF </td>
		  ...
		  onKeyUp="return tabular(event,this)" value="<?php echo $_POST[numero]?>" onClick="document.getElementById('numero').focus();document.getElementById('numero').select();">
		 	...
		 	<td style="width:2.5cm" class="saludo1">Nombre:</td>
          	<td style="width:40%" valign="middle" >
            	<input type="text" id="nombre" name="nombre" style="width:100%"onKeyUp="return tabular(event,this)" value="<?php echo $_POST[nombre]?>" onClick="document.getElementById('acuerdo').focus();document.getElementById('acuerdo').select();">
           	</td>
          	<td style="width:2.5cm" class="saludo1">Tipo:</td>
          	<td style="width:15%">
            	<input name="tipo" value="Predial" style="width:100%" type="text">
                <input name="tipoc" value="IS" size="10" type="hidden"><input name="ban" type="hidden"  value="1">
          	</td>
	  	</tr>
    </table>
	<table class="inicio">
		...
		?>
        	...
        	<td>
            	<select name="debcred" style="width:90%;">
	...
	if ($_POST[agregadet]=='1')
		 ...
		 {echo "<tr class='$iter' onMouseOver=\"anterior=this.style.backgroundColor;this.style.backgroundColor='#40b3ff';\" onMouseOut=\"this.style.backgroundColor=anterior\">
			<td>
				<input name='fecha[]' value='".$_POST[fecha][$x]."' type='text' size='10' readonly class='inpnovisibles'>
			</td>
		 	<td>
				<input name='dccs[]' value='".$_POST[dccs][$x]."' type='text' size='2' readonly class='inpnovisibles'>
			</td>
			<td>
				<input name='dcuentas[]' value='".$_POST[dcuentas][$x]."' type='text' size='8' readonly class='inpnovisibles'>
			</td>
			<td>
				<input name='dncuentas[]' value='".$_POST[dncuentas][$x]."' type='text' size='70' readonly class='inpnovisibles'>
			</td>
			<td>
				<input name='ddebitos[]' value='".$_POST[ddebitos][$x]."' type='text' size='3' onDblClick='llamarventanadeb(this,$x)' readonly class='inpnovisibles'>
			</td>
			<td>
				<input name='dcreditos[]' value='".$_POST[dcreditos][$x]."' type='text' size='3' onDblClick='llamarventanacred(this,$x)' readonly class='inpnovisibles'>
			</td>
		...
		for($x=0;$x<count($_POST[dcuentas]);$x++) 
		 {
		  ...
		  $sqlr="insert into conceptoscontables_det (codigo,tipo,tipocuenta,cuenta,cc,debito,credito,estado,modulo,fechainicial) values ('$_POST[numero]','$_POST[tipoc]','N','".$_POST[dcuentas][$x]."','".$_POST[dccs][$x]."','".$_POST[ddebitos][$x]."','".$_POST[dcreditos][$x]."','S','4','".$_POST[fecha][$x]."')";
		  $res=mysql_query($sqlr,$linkbd);
		 }
	   }
	?>	
