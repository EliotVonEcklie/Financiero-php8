var app = new Vue({ 
	...
	{
		this.fetchMembers();
		this.idinicial(this.idproyecto);
		this.cargayears();
	},
	computed:
	{
	...
	methods:
	{
		...
		idinicial: function(idini)
		{
			this.inicioproyecto(idini);
			this.iniciosector(idini);
			this.iniciprograma(idini);
			this.inicioproductos(idini);
			this.iniciosinclasificador(idini);
			this.inicioclasificador1(idini);
			this.inicioclasificador2(idini);
			this.inicioclasificador3(idini);
		},
		inicioproyecto: async function(idini)
		{
			await axios.post('vue/presupuesto_ccp/ccp-bancoproyectoseditar.php?accion=inicialproyecto&idproy='+idini)
			.then(
				(response)=>
				{
		...
		iniciaunieje: async function(idunieje)
		{
			await axios.post('vue/presupuesto_ccp/ccp-bancoproyectoseditar.php?accion=unidadejecutora&idunieje='+idunieje)
			.then(
				(response)=>
				{
					this.unidadejecutora = idunieje + " - " + response.data.codigos[0][0];
					this.cunidadejecutora = idunieje;
				}
			);
		},
		iniciosector: async function(idini)
		{
			await axios.post('vue/presupuesto_ccp/ccp-bancoproyectoseditar.php?accion=iniciasector&idproy='+idini)
			.then(
				(response)=>
				{
		...
		{
			await axios.post('vue/presupuesto_ccp/ccp-bancoproyectoseditar.php?accion=iniciaprograma&idproy='+idini)
			.then(
				(response)=>
				{
		...
		{
			await axios.post('vue/presupuesto_ccp/ccp-bancoproyectoseditar.php?accion=iniciaproductos&idunieje='+idunieje)
			.then(
				(response)=>
				{ 
		...
		iniciosinclasificador: async function(idini)
		{
			await axios.post('vue/presupuesto_ccp/ccp-bancoproyectoseditar.php?accion=iniciasinclasificador&idproy='+idini)
			.then(
				(response)=>
				{
					for(var x=0; x < response.data.codigos.length;x++)
					{
						var nombrefuente = response.data.codigos[x][0];
						var tipodepago = response.data.codigos[x][1];
						var codigorubro = response.data.codigos[x][2];
						var nombrerubro = response.data.codigos[x][2] + " - " + response.data.codigos[x][5];
						if (tipodepago == 'CSF'){var valorsinclas = response.data.codigos[x][3];}
						else {var valorsinclas = response.data.codigos[x][4];}
						var varauxia=[codigorubro,nombrerubro,valorsinclas,nombrefuente,tipodepago ];
						var varauxib=[codigorubro,valorsinclas,nombrefuente,tipodepago];
		...
		inicioclasificador1: async function(idini)
		{
			await axios.post('vue/presupuesto_ccp/ccp-bancoproyectoseditar.php?accion=iniciaclasificador1&idproy='+idini)
			.then(
				(response)=>
				{
					for(var x=0; x < response.data.codigos.length;x++)
					{
						var codfuente = response.data.codigos[x][0];
						var nombrefuente = response.data.codigos[x][0] + " - " + response.data.codigos[x][9];
						var tipodepago = response.data.codigos[x][1];
						var codigorubro = response.data.codigos[x][2];
						var identidad = response.data.codigos[x][3];
						var nitentidad = response.data.codigos[x][4];
						var nomentidad = response.data.codigos[x][8];
						var numcuin = response.data.codigos[x][5];
						if (tipodepago == 'CSF'){var valorclas1 = response.data.codigos[x][6];}
						else {var valorclas1 = response.data.codigos[x][7];}
						var varauxia=[identidad,nitentidad,nomentidad,numcuin,valorclas1,nombrefuente,tipodepago,codigorubro];
						var varauxib=[codigorubro,identidad,nitentidad,numcuin,valorclas1,codfuente,tipodepago];
		...
		inicioclasificador2: async function(idini)
		{
			await axios.post('vue/presupuesto_ccp/ccp-bancoproyectoseditar.php?accion=iniciaclasificador2&idproy='+idini)
			.then(
				(response)=>
				{
					for(var x=0; x < response.data.codigos.length;x++)
					{
						var codfuente = response.data.codigos[x][0];
						var nombrefuente = response.data.codigos[x][0] + " - " + response.data.codigos[x][11];
						var tipodepago = response.data.codigos[x][1];
						var codigorubro = response.data.codigos[x][2];
						var nombrerubro = response.data.codigos[x][2] + " - " + response.data.codigos[x][10];
						var codseccion = response.data.codigos[x][3];
						var coddivicion = response.data.codigos[x][4];
						var codgrupo = response.data.codigos[x][5];
						var codclase = response.data.codigos[x][6];
						var codsubclase = response.data.codigos[x][7];
						var nomsubclase = response.data.codigos[x][7] + " - " + response.data.codigos[x][12];
						var subproducto = response.data.codigos[x][13];
						if (tipodepago == 'CSF'){var valorclas1 = response.data.codigos[x][8];}
						else {var valorclas1 = response.data.codigos[x][9];}
						var unionaux = '2' + '<->' + codigorubro + '<->' + codsubclase;
						var varauxia=['2',nombrerubro,nomsubclase,valorclas1,nombrefuente,tipodepago,subproducto];
						var varauxib=[codigorubro,'2',codseccion,coddivicion,codgrupo,codclase, codsubclase,valorclas1,codfuente,tipodepago,subproducto];
		...
		inicioclasificador3: async function(idini)
		{
			await axios.post('vue/presupuesto_ccp/ccp-bancoproyectoseditar.php?accion=iniciaclasificador3&idproy='+idini)
			.then(
				(response)=>
				{
					for(var x=0; x < response.data.codigos.length;x++)
					{
						...
						var unionaux = '3' + '<->' + codigorubro + '<->' + codsubclase;
						var varauxia=['3',nombrerubro,nomsubclase,valorclas1,nombrefuente,tipodepago,subproducto];
						var varauxib=[codigorubro,'3',codseccion,coddivicion,codgrupo,codclase, codsubclase,valorclas1,codfuente,tipodepago,subproducto];
						var buscaraux=[unionaux];
						this.selectcuetasa.push(varauxia);
						this.selectcuetasb.push(varauxib);
						this.selectbuscar.push(unionaux);
										...
										if(this.valorrubro > 0 )
										{
											if(!this.valorrubro.includes('.'))
											{
												if(bandera01==0)
												{
													...
													this.selectcuetasb.push(varauxib);
													this.selectbuscar.push(unionaux);
													this.csubproducto=this.subproducto=this.cseccion=this.cdivision=this.cgrupo=this.cclase=this.csubclase= this.seccion=this.division=this.grupo=this.clase=this.subclase=this.valorrubro=this.nrubro=this.codrubro='';
													this.clasificador=0;
												}
												else
												{
		...
		guardarcabecera: function(idnum)
		{
			...
			axios.post('vue/presupuesto_ccp/ccp-bancoproyectoseditar.php?guardar=cabecera', formData)
			.then();
		},
		guardarproductos: function(idnum)
		{
			...
			axios.post('vue/presupuesto_ccp/ccp-bancoproyectoseditar.php?guardar=productos', formData)
			.then();
		},
		guardarpresupuestob: function(idnum)
		{
			...
			axios.post('vue/presupuesto_ccp/ccp-bancoproyectoseditar.php?guardar=cuentasb', formData)
			.then();
		},
		...
		guardarsinclasificador: function(idnum)
		{
			...
			axios.post('vue/presupuesto_ccp/ccp-bancoproyectoseditar.php?guardar=ccuentassinclasificador', formData)
			.then();
		},
