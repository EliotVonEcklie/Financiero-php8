...
if ($_POST[estadoc]=='ANULADO'){
		    $this->Image('imagenes/anulado.jpg',50,15,150,80);
	}
   //Parte Izquierda
...
while($row=mysql_fetch_row($res))
 ...
 $this->Image('imagenes/eng.jpg',23,10,25,25);
	$this->SetFont('Arial','B',10);
	$this->SetY(10);
	$this->RoundedRect(10, 10, 195, 31, 1,'' );
	$this->Cell(0.1);
	...
	$this->SetY(10);
    $this->Cell(50.1);
    $this->Cell(140,25,'RECIBO INGRESOS SERVICIOS PUBLICOS',0,0,'C'); 
	//************************************
    $this->SetFont('Arial','B',10);
	...
	$tiporc=$_POST[tiporec];
	$this->SetY(26);
	$this->Cell(50.2);
	$this->multiCell(110.7,8,''.substr($_POST[concepto],0,100),'T','L');
	$this->Cell(50.2);
	$this->multiCell(106.7,5,'Liquidacion: '.$tipos[$_POST[trec]-1].' No '.substr($_POST[idrecaudo],0,60),'T','L');
...
$pdf->RoundedRect(157, 46 ,48 , 8, 1,'' );
$pdf->cell(48,8,'$'.$_POST[totalcf],0,0,'R');
...
$pdf->SetY(55);
$pdf->cell(0.2);
$pdf->cell(29,5,'RECIBIMOS DE: ',0,0,'L',1);
...
$pdf->cell(35,5,'FORMA DE PAGO: ',0,0,'L',1);
$pdf->RoundedRect(10, 55 ,195 , 22, 1,'' );

//$pdf->cell(4,5,'','1',0,'C',1);
//$pdf->cell(0.8);
$pdf->cell(19,5,'EFECTIVO ',0,0,'L',1);
if($_POST[modorec]=='caja')
{$pdf->cell(4,5,'X','1',0,'C',1);
...
$pdf->cell(0.7);
$pdf->cell(15,5,'BANCO ',0,0,'L',1);
if($_POST[modorec]=='banco')
{$pdf->cell(4,5,'X','1',0,'C',1);
$pdf->cell(116,5,''.$_POST[cb]." ".$_POST[nbanco],0,0,'',1);
}
else
{$pdf->cell(4,5,'','1',0,'C',1);$pdf->cell(116,5,'',0,0,'',1);}
...
{	if ($con%2==0)
	...
	if ($_POST[dcoding][$con]!='')
	 {
	$pdf->Cell(156,4,''.$_POST[dcoding][$con]." - ".substr(ucwords(strtolower($_POST[dncoding][$con])),0,70),0,0,'L',1);//descrip
	$pdf->Cell(39,4,''.number_format($_POST[dvalores][$con],2),0,0,'R',1);
	 }
	 else
	 {
		 $pdf->Cell(156,4,'',0,0,'L',1);//descrip
	$pdf->Cell(39,4,'',0,0,'L',1);
		 }
	$pdf->ln(4);
 $con=$con+1;   
	}
...
$niy=$pdf->Gety();
$pdf->RoundedRect(10, 78 ,195 ,$niy-78 , 1,'' );
