...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="es">
		...
		<script>
			function verUltimaPos(idcta, filas, filtro){
				...
				if((limreg==0)||(limreg==""))
					limreg=10;
				numpag++;
				location.href="meci-editarareas.php?idtipocom="+idcta+"&scrtop="+scrtop+"&totreg="+filas+"&altura="+altura+"&numpag="+numpag+"&limreg="+limreg+"&filtro="+filtro;
			}
		</script>
        <script>
            function cambioswitch(id,valor)
			{
				...
				if(valor==1)
				{
					despliegamodalm('visible','4','Desea Activar esta Area','1');
				}
				else
				{
					despliegamodalm('visible','4','Desea Desactivar esta Area','2');
				}
			}
	...
	<body>
		...
		?>
 		<form name="form2" method="post" action="meci-buscaareas.php">
        	<?php
				if($_POST[oculto]=="")
				{
					$_POST[cambioestado]="";
			...
			<table  class="inicio ancho" align="center" >
      			<tr>
        			<td class="titulos" colspan="2">:: Buscar Areas </td>
        			<td class="boton02" onclick="location.href='meci-principal.php'">Cerrar</td>
      			</tr>
                <tr><td class="titulos2" colspan="3">:&middot; Por Descripción </td></tr>
              	<tr>
    		...
    		<div class="subpantalla" style="height:66%; width:99.8%; overflow-x:hidden;" id="divdet">
      			<?php
					$crit1="";
					if ($_POST[nombre]!="")
					$crit1="and concat_ws(' ', id_cc, nombre) LIKE '%$_POST[nombre]%'";
					$sqlr="SELECT * FROM admareas WHERE estado<>'' $crit1";
					...
					$sqlr="SELECT * FROM admareas WHERE estado<>'' $crit1 ORDER BY id_cc ".$cond2;
					$resp = mysql_query($sqlr,$linkbd);
					...
					$filas=1;
 					while ($row =mysql_fetch_row($resp)) 
 					{
						...
						echo"<tr class='$iter' onDblClick=\"verUltimaPos($idcta, $numfil, $filtro)\" style='text-transform:uppercase; $estilo' >
							<td class='icoop centrartext'>$row[0]</td>
