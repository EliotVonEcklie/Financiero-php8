...
<?php
	...
	class MYPDF extends TCPDF 
	{
		public function Header() 
		{
			...
			$this->SetY(10);
			$this->SetX(62);
			$this->Cell(190,17,'Certificados Disponibilidad Presupuestal Reversados',0,0,'C'); 
			...
			$this->Cell(40,5,'Valor CDP',1,0,'C',false,0,0,false,'T','C');
			$this->Cell(40,5,'Valor Reintegrado',1,0,'C',false,0,0,false,'T','C');
			$this->Cell(70,5,'Reintegrado Por',1,0,'C',false,0,0,false,'T','C');
			$this->Cell(70,5,'Detalle',1,0,'C',false,0,0,false,'T','C');
			$this->Cell(20,5,'Fecha',1,0,'C',false,0,0,false,'T','C');
		}
...
EOD;
	...
	$sqlr="SELECT TB1.*,TB2.user,TB2.detalle FROM pptocdp TB1, pptocdp_cab_r TB2 WHERE TB1.estado='R' AND TB1.vigencia=TB2.vigencia AND TB1.consvigencia=TB2.consvigencia AND TB2.tipo_mov='02' $crit1 $crit2 $crit3 ORDER BY TB1.consvigencia";
	$resp = mysql_query($sqlr,$linkbd);
	while ($row =mysql_fetch_row($resp))
	{
		...
		$pdf->Cell(20,$altura,$row[2],1,0,'C',true,0,0,false,'T','C');
		$pdf->Cell(40,$altura,"$ ".number_format((float)$row[4],2,",",".")." ",1,0,'R',true,0,0,false,'T','C');
		$pdf->Cell(40,$altura,"$ ".number_format((float)$row1[0],2,",",".")." ",1,0,'R',true,0,0,false,'T','C');
		...
		$pdf->Cell(20,$altura,date('d-m-Y',strtotime($row[3])),1,1,'C',true,0,0,false,'T','C');
	}
	// ---------------------------------------------------------
	$pdf->Output('reporterpr.pdf', 'I');//Close and output PDF document
?>