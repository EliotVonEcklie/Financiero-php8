...
<?php
	...
	if($_POST[oculto]=='2')
	{
		...
		//-----reemplazar en pptocomprobante_cab
		...
		mysql_query($sqlr,$linkbd);
	 	...
	 	//-----reemplazar en pptocomprobante_det
	 	//$diddet[]=array();
	 	$cuentadet[]=array();
	 	//$tercerodet[]=array();
	 	//$detalledet[]=array();
	 	...
	 	$i=0;
	 	$sqlr="select * from pptocomprobante_det where tipo_comp=17 and numerotipo=$ideg";
		...
		mysql_query($sqlr,$linkbd);
		/*for($j=0;$j<count($diddet);$j++){
			
			$sqlr="insert into  pptocomprobante_det (cuenta,tercero,detalle,valdebito,valcredito,estado,vigencia,tipo_comp,numerotipo) values('".$cuentadet[$j]."','".$tercerodet[$j]."','DESCUENTO EGRESO ',".$valdebitodet[$j].",0,1,'$_POST[vigencia]',17,'$ideg')";
			mysql_query($sqlr,$linkbd); 
			$sqlr="insert into pptoretencionpago  (cuenta,idrecibo,valor,vigencia) values ($cuentadet[$j],$ideg,".$valdebitodet[$j].",".$vigenciadet[$j].")";				
			mysql_query($sqlr,$linkbd);

			$sqlr="UPDATE  pptocuentaspptoinicial SET ingresos=ingresos+".$valdebitodet[$j]." where (vigencia='".$vigusu."' or vigenciaf='$vigusu') and cuenta='$cuentadet[$j]' ";	
			mysql_query($sqlr,$linkbd);
		}*/
		...
		{																		 			 
			...
			{							  
	   			...
	   			if($rowdes[3]!="")
				{
					...
					$sqlr="insert into pptoretencionpago  (cuenta,idrecibo,valor,vigencia) values ($rowdes[3],$ideg,".$valdebitodet[$x].",".$_POST[vigencia].")";				
					mysql_query($sqlr,$linkbd);
					$sqlr="UPDATE  pptocuentaspptoinicial SET ingresos=ingresos+".$valdebitodet[$x]." where (vigencia='".$vigusu."' or vigenciaf='$vigusu') and cuenta='$rowdes[3]' ";	
					mysql_query($sqlr,$linkbd);		
					$sqlr="insert into  pptocomprobante_det (cuenta,tercero,detalle,valdebito,valcredito,estado,vigencia,tipo_comp,numerotipo) values('".$rowdes[3]."','".$_POST[tercero]."','DESCUENTO EGRESO ',".$valdebitodet[$x].",0,1,'$_POST[vigencia]',17,'$ideg')";
					mysql_query($sqlr,$linkbd); 	
					//echo "$sqlr<br>";

					//$sqlr="update pptocomprobante_det set cuenta='".$rowdes[3]."', tercero='".$_POST[tercero]."' where cuenta='".$cuentadet[$x]."'";
					//echo $sqlr;
					//mysql_query($sqlr,$linkbd);
				}
			}							  
