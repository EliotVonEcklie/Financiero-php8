...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="es">
		...
		<script>
			function verUltimaPos(idcta, filas, filtro){
				...
				if((limreg==0)||(limreg==""))
					limreg=10;
				numpag++;
				location.href="meci-editaprocesos.php?idproceso="+idcta+"&scrtop="+scrtop+"&totreg="+filas+"&altura="+altura+"&numpag="+numpag+"&limreg="+limreg+"&filtro="+filtro;
			}
		</script>
		<script>
			function cambioswitch(id,valor)
			{
				...
				if(valor==1){despliegamodalm('visible','4','Desea Desactivar este Proceso','1');}
				else{despliegamodalm('visible','4','Desea Activar este Proceso','2');}
			}
	...
	<body>
		...
		?>
 		<form name="form2" method="post" action="meci-buscaprocesos.php">
            ...
            <?php
				...
				if($_POST[cambioestado]!="")
				{
					...
					{
						$sqlr ="UPDATE calprocesos SET estado='S' WHERE  id='$_POST[idestado]'";
						mysql_fetch_row(mysql_query($sqlr,$linkbd)); 
					}
					else 
					{
						$sqlr ="UPDATE calprocesos SET estado='N' WHERE id='$_POST[idestado]'";
						mysql_fetch_row(mysql_query($sqlr,$linkbd)); 
					}
			...
			<table class="inicio ancho">
      			<tr>
        			<td class="titulos" colspan="2">:: Buscar Procesos </td>
    		...
    		<div class="subpantallac5" style="height:69.5%; width:99.6%; overflow-x:hidden;" id="divdet">
      			<?php
					...
					$sqlr="SELECT * FROM calprocesos WHERE estado<>'' $crit1";
					...
					$sqlr="SELECT * FROM calprocesos WHERE estado<>'' $crit1 ORDER BY id ".$cond2;
					$resp = mysql_query($sqlr,$linkbd);
					$con=1;
					...
					$filas=1;
 					...
 					{
						if($row[4]=='S') {$coloracti="#0F0";$_POST[lswitch1][$row[0]]=0;}
						else {$coloracti="#C00";$_POST[lswitch1][$row[0]]=1;}
						if($gidcta!=""){
							if($gidcta==$row[0]){
						...
						echo"<tr class='$iter'  onDblClick=\"verUltimaPos($idcta, $numfil, $filtro)\" style='text-transform:uppercase; $estilo' >
							...
							<td class='icoop' width='80%'>$row[1]</td>
							<td class='icoop centrartext'>$row[3] </td>
							<td><input type='range' name='lswitch1[]' value='".$_POST[lswitch1][$row[0]]."' min ='0' max='1' step ='1' style='background:$coloracti; width:80%' onChange='cambioswitch(\"$row[0]\",\"".$_POST[lswitch1][$row[0]]."\")' /></td>
							</tr>";
						$con+=1;
						$aux=$iter;
						$iter=$iter2;
