...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="es">
	<head>
		...
		<script type="text/javascript" src="JQuery/jquery.ripples.js"></script>
	...
	<body>
		<video autoplay muted loop id="myVideo">
			<source src="imagenes/acuario.mp4" type="video/mp4">
		</video>
		<form name="form1" method="post" action="index2.php"> 
			...
			<div  style="margin: 15px auto; padding-bottom: 30px; padding-top:30px; padding-left: 250px; padding-right: 250px;   width:35%;   ">
				<div><img src="imagenes/FONDO-2.png" align="absmiddle" style="width:100%; margin-bottom:10px; margin-top: 50px;"></div>
				<table class="inicio"  style=" padding: 0; position: relative; width: 100%;height: 15.5em; list-style: none; border: 3px solid #eee; border-radius: 5px; box-shadow: 0 0 10px #000; overflow: hidden;">
					<tr>
						<td style="border-bottom:#222 thin dotted; background-color:rgba(44, 198, 219); color:#FFFFFF; width:100%" colspan="3"><image src='imagenes/llaves.png' style=" height:40%; width:12%; "> Inicio de Sesion :.</td>	
					</tr>
					<tr>
						<td style="border-bottom:#222 thin dotted; background-color: rgba(75, 231, 89); text-align:center; width:10%; height:20%" colspan="1"><image src='imagenes/usuario_on.png' style=" height:20%; width:60%;"></td>
						<td style="border-bottom:#222 thin dotted; background-color: #eeeeee;"> Usuario :.</td> 
						<td style="border-bottom:#222 thin dotted; text-align:center"><input type="text" name="user" id="user" value="<?php echo @$_POST['user'];?>" style="width:65%; height:90%"/></td>
					</tr>
					<tr>
						<td style="border-bottom:#222 thin dotted; background-color: rgba(75, 231, 89); text-align:center; width:10%; height:20%" colspan="1"><image src='imagenes/candado.png' style=" height:16%; width:50%;"></td> <td style="border-bottom:#222 thin dotted; background-color: #CCCCCC;">Contrase&ntilde;a :.</td>
						<td style=" text-align:center;border-bottom:#222 thin dotted;"><input type="password" name="pass" id="pass" value="<?php echo @$_POST['pass'];?>" style="width:65%; height:90%" /></td>
					</tr>
					<tr>
				...
				<table class="inicio"  style="visibility:<?php echo @$_POST['vistab'];?>;">
					<tr>
						<td>
							<select name="basesfun" id="basesfun">
								<?php
									...
									{
										if("$row[0]"==@$_POST['basesfun']){echo "<option value='$row[0]' SELECTED>$row[0]</option>";}
			...
			<div style="margin-top:7%;">
				...
				<?php
					if (@$_POST['user']!="" && @$_POST['pass']!="" && @$_POST['oculto']=="1")
					{
						...
						if ($valap == ""){echo"<script>despliegamodalm('visible','Error el Usuario o la Contraseña no son correctos');</script>";}
						else {echo"<script>document.form1.action='principal.php';document.form1.submit();</script>";}
					}
					else if(@$_POST['user']=="" && @$_POST['oculto']=="1"){echo"<script>despliegamodalm('visible','Debe Ingresar Nombre del Usuario');</script>";}
					else if(@$_POST['oculto']=="1"){echo"<script>despliegamodalm('visible','Debe Ingresar La Clave Asignada');</script>";}
				?>
			</div>
			<?php
				if(@$_POST['oculto']=="4")
