...
document.form2.action="cont-sinrecaudos-reflejarsp.php";
document.form2.submit();
}
}
</script>
...
if(!$_POST[oculto]){
	...
	if ($_POST[codrec]!="" || $_GET[idr]!=""){
		if($_POST[codrec]!=""){
			$sqlr="select * from tesosinrecaudossp WHERE ID_RECAUDO='$_POST[codrec]'";
		}
		else{
			$sqlr="select * from tesosinrecaudossp WHERE ID_RECAUDO='$_GET[idr]'";
		}
	}
	else{
		$sqlr="select * from tesosinrecaudossp ORDER BY id_recaudo DESC";
...
if($_POST[oculto]=='2')
{
	...
	//***busca el consecutivo del comprobante contable
		...
		{
		$sqlr="update comprobante_cab set estado=0 where tipo_comp=29 and numerotipo=$_POST[idcomp]";
		mysql_query($sqlr,$linkbd);			
		$sqlr="update comprobante_det set valcredito=0, valcredito=0 where tipo_comp=29 and numerotipo=$_POST[idcomp]";
		mysql_query($sqlr,$linkbd);		
		}
		else
		{
		...
		$sqlr="update comprobante_cab set estado=1 where tipo_comp=29 and numerotipo=$_POST[idcomp]";
		mysql_query($sqlr,$linkbd);			
		$sqlr="delete from comprobante_det where id_comp='29 $_POST[idcomp]'";
		mysql_query($sqlr,$linkbd);
			...
			while($rowc=mysql_fetch_row($resc))
			 {
				...
				$sqlr="insert into comprobante_det (id_comp,cuenta,tercero,centrocosto,detalle,cheque,valdebito,valcredito,estado,vigencia) values ('29 $_POST[idcomp]','".$rowc[4]."','".$_POST[tercero]."','".$rowc[5]."','Causacion ".$_POST[dncoding][$x]."','',".$valordeb.",".$valorcred.",'1','".$_POST[vigencia]."')";
				mysql_query($sqlr,$linkbd);
				//echo "Conc: $sqlr <br>";
			 }
			 }
