...
while($row=mysql_fetch_row($res))
 {
  ...
  $rs=$row[1];
	...
	$this->SetY(8);
    $this->Cell(50.1);
    $this->Cell(149,20,'LISTADO DISPONIBILIDADES PRESUPUESTALES',0,0,'C'); 
	//************************************
    $this->SetFont('Arial','B',10);
...
$pdf->SetAutoPageBreak(true,20);
	...
	$pdf->Cell(199,4,'LISTADO CDP',0,0,'C');
 $pdf->ln(8); 
	 $y=$pdf->GetY();	
	...
	$pdf->SetY($y);
    $pdf->Cell(0.1);
    $pdf->Cell(18,5,'VIGENCIA ',0,0,'C',1); 
		$pdf->SetY($y);
    	$pdf->Cell(19.1);
		$pdf->Cell(15,5,'CDP',0,0,'C',1);
		$pdf->SetY($y);
    	$pdf->Cell(35.1);
		$pdf->Cell(90,5,'OBJETO',0,0,'C',1);
		$pdf->SetY($y);
    	$pdf->Cell(126.1);
		$pdf->Cell(32.5,5,'VALOR',0,0,'C',1);
		$pdf->SetY($y);
       	$pdf->Cell(159.5);
 ...
 for($x=0;$x<count($_POST[vigencias]);$x++)
	 {	
	  ...
	  {$pdf->SetFillColor(245,245,245);
		...
		$pdf->Cell(22,4,''.$_POST[vigencias][$x],'',0,'C',1);
		$pdf->Cell(12,4,''.$_POST[cdps][$x],'',0,'C',1);
		$pdf->Cell(90,4,''.substr($_POST[objetos][$x],0,70),'',0,'L',1);
		$pdf->Cell(35,4,''.$_POST[valores][$x],'',0,'R',1);
		$pdf->Cell(25,4,''.$_POST[fechas][$x],'',0,'C',1);
