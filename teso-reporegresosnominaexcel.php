<?php 
	...
	$objPHPExcel->setActiveSheetIndex(0)
		...
		->mergeCells('A2:K2')
     	...
     	->setCellValue('A2', 'AUXILIAR DE EGRESOS DE NOMINA - Periodo: '.$_GET[fecha1].' - '.$_GET[fecha2]);
	...
	$cont=4;
    $sqlr2="select *from tesoegresosnomina where tesoegresosnomina.id_egreso>-1 AND FECHA BETWEEN '$fechaf' AND '$fechaf2' order by tesoegresosnomina.id_egreso DESC";

	$resp = mysql_query($sqlr2,$linkbd);
	while ($row =mysql_fetch_row($resp)) 
	{
		$ntr=buscatercero($row[11]);
		$objWorksheet->fromArray(array($row[0],$row[2],$row[11],utf8_encode($ntr),date('d-m-Y',strtotime($row[3])),$row[10],number_format($row[5],2),number_format($row[7],2),number_format($row[6],2),strtoupper("EGRESOS NOMINA".$row[8]),$row[13]),NULL,"A$cont");
	...
	header('Content-Disposition: attachment;filename="LIBRO_EGRESO_NOMINA.xlsx"');
