...
function pdf()
{
document.form2.action="pdfreporegresosnomina.php";
...
<table>
	...
	<tr>
		<td colspan="3" class="cinta">
			...
			<a href="<?php echo "archivos/".$_SESSION[usuario]."-reporteegresos.csv"; ?>" target="_blank" class="mgbt"><img src="imagenes/csv.png"  alt="csv" title="csv"></a>
...
<tr><td colspan="3" class="tablaprin"> 
 <form name="form2" method="post" action="teso-reporegresosnomina.php">
 <?php
 if($_POST[bc]=='1')
			 {
...
<table  class="inicio" align="center" >
      ...
      <tr >
        <td class="titulos" colspan="8">:. Reporte egresos n&oacute;mina</td>
...
$crit2=" ";
if ($_POST[tercero]!="")
$crit3=" and tesoegresosnomina.tercero like '%".$_POST[tercero]."%' ";
if ($_POST[numero]!="")
...
//$sqlr="select *from pptosideforigen where".$crit1.$crit2." order by pptosideforigen.codigo";
	$sqlr="select *from tesoegresosnomina where tesoegresosnomina.id_egreso>-1 ".$crit1.$crit2.$crit3." AND FECHA BETWEEN '$fechaf' AND '$fechaf2' order by tesoegresosnomina.id_egreso DESC";
...
fputs($Descriptor1,"EGRESO;ORDEN PAGO;Doc Tercero;TERCERO;FECHA;VALOR;CONCEPTO;ESTADO\r\n");
echo "<table class='inicio' align='center' ><tr><td colspan='8' class='titulos'>.: Resultados Busqueda:</td></tr><tr><td colspan='8' class='saludo3'>Pagos Encontrados: $ntr</td></tr><tr><td  class='titulos2'>Egreso</td><td  class='titulos2'>Orden Pago</td><td class='titulos2'>Doc Tercero</td><td class='titulos2'>Tercero</td><td class='titulos2'>Fecha</td><td class='titulos2'>Valor</td><td class='titulos2'>Concepto</td><td class='titulos2' width='3%'><center>Estado</td></tr>";	
//echo "nr:".$nr;
$iter='saludo1';
$iter2='saludo2';
 while ($row =mysql_fetch_row($resp)) 
 {
	 ...
	 echo "<tr class='$iter'><td ><input type='hidden' name='egresos[]' value='$row[0]'>$row[0]</td><td ><input type='hidden' name='ordenes[]' value='$row[2]'>$row[2]</td><td ><input type='hidden' name='terceros[]' value='$row[11]'>$row[11]</td><td ><input type='hidden' name='nterceros[]' value='$ntr'>$ntr</td><td ><input type='hidden' name='fechas[]' value='$row[3]'>$row[3]</td><td ><input type='hidden' name='valores[]' value='$row[7]'>".number_format($row[7],2)."</td><td ><input type='hidden' name='conceptos[]' value='$row[8]'>".strtoupper($row[8])."</td><td ><input type='hidden' name='estados[]' value='$row[13]'>".strtoupper($row[13])."</td></tr>";
	 $con+=1;
	 $aux=$iter;
	 $iter=$iter2;
	 $iter2=$aux;
